        ...
        "supported_spec_split": {
            ...
            "helper_boundary": {
                ...
                "next_required_proof_step": (
                    ...
                    "Dispatch.runtimeContractOfFunctions. That packages the "
                    "remaining stmt seam as one non-recursive expr theorem plus "
                    "recursive `if` / `block` transport on the legacy-"
                    "compatible external-body Yul subset. The full interface "
                    ...
                    "expr/list plumbing: it is concentrated in the special "
                    "expression-statement cases (`sstore`, `mstore`, `return`, "
                    "`revert`) plus nested `if`/`block` transport on that "
                    "Yul subset, "
