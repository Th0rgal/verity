...
class NativeTransitionDocCheckTests(unittest.TestCase):
    ...
    def test_public_theorem_target_guard_rejects_public_lowering_witness_name(self) -> None:
        end_to_end_text = self.replace_in_theorem_signature(
            check.END_TO_END.read_text(encoding="utf-8"),
            "nativeGeneratedCallDispatcherMatchesIR_of_compile_ok_supported",
            "hUserBodyHalt",
        ...
        self.assertTrue(
            any("hLowerRuntime" in error for error in errors),
            errors,
        )
    ...
    def test_native_closure_import_boundary_rejects_missing_source_storage_closure(self) -> None:
        source_text = check.SOURCE_EXPR_CLOSURE.read_text(encoding="utf-8").replace(
        ...
        self.assertTrue(
            any("BridgedSourceExpr (.storage fieldName)" in error for error in errors),
            errors,
        )

    def test_native_closure_import_boundary_rejects_missing_source_array_length_closure(self) -> None:
        source_text = check.SOURCE_EXPR_CLOSURE.read_text(encoding="utf-8").replace(
            "| arrayLength (name : String) : BridgedSourceExpr (.arrayLength name)",
            "| arrayLengthRemoved (name : String) : BridgedSourceExpr (.arrayLength name)",
            1,
        )
        errors = check.check_native_closure_import_boundary(
            check.BRIDGE_PREDICATES.read_text(encoding="utf-8"),
        ...
        self.assertTrue(
            any("arrayLength" in error for error in errors),
            errors,
        )
    ...
    def test_native_closure_import_boundary_rejects_missing_source_storage_array_length_closure(self) -> None:
        source_text = check.SOURCE_EXPR_CLOSURE.read_text(encoding="utf-8").replace(
            ...
            "| storageArrayLengthRemoved (fieldName : String) :",
            1,
        )
        errors = check.check_native_closure_import_boundary(
            check.BRIDGE_PREDICATES.read_text(encoding="utf-8"),
        ...
        self.assertTrue(
            any("storageArrayLength" in error for error in errors),
            errors,
        )
