...
| EVMYulLean bridge | `Compiler/Proofs/YulGeneration/Backends/EvmYulLeanBridgeLemmas.lean` | Universal bridge lemmas for all 20 pure builtins: `add`/`sub`/`mul`/`div`/`mod`, `addmod`/`mulmod`, `lt`/`gt`/`slt`/`sgt`/`eq`/`iszero`, `and`/`or`/`xor`/`not`, `shl`/`shr`, and `byte` |
