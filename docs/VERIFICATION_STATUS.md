...
- the machine-readable boundary catalog now makes that blocker exact rather than implicit: callers still reach the body theorem through the helper-free `SupportedStmtList` witness, the helper-aware body theorem does not yet consume helper-summary soundness/rank evidence end to end, and `execIRFunction` has not yet been extended with internal helper call composition. The compiled-side blocker is tracked in [#1638](https://github.com/Th0rgal/verity/issues/1638)
