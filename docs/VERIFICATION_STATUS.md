...
Trust boundary (public EndToEnd target): native `EvmYul.Yul.callDispatcher` execution is the public semantic target. The remaining backend-wrapper retargeting facts still mention `execYulFuelWithBackend`/`interpretYulRuntimeWithBackend .evmYulLean` as lower-level transition evidence for the bridged safe-body fragment, but `EndToEnd.lean` does not export or privately compose compiler-correctness theorems over that target.
