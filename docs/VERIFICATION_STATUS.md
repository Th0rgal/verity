...
Native-runtime transition status: the current theorem target is still the backend-parameterized Verity statement interpreter. The executable native EVMYulLean path lives in [`EvmYulLeanNativeHarness.lean`](../Compiler/Proofs/YulGeneration/Backends/EvmYulLeanNativeHarness.lean), and the remaining theorem-transition plan is tracked in [`NATIVE_EVMYULLEAN_TRANSITION.md`](NATIVE_EVMYULLEAN_TRANSITION.md). The generic native seams `layer3_contract_preserves_semantics_native_via_reference_oracle_of_evmYulLean_bridge` and `layers2_3_ir_matches_native_evmYulLean_via_reference_oracle_of_evmYulLean_bridge` explicitly name the current dependency on the reference-oracle retarget plus EVMYulLean fuel-wrapper comparison; their shorter compatibility theorem names delegate to those explicit seams.
