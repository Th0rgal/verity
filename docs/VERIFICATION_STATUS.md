...
- Indexed `bytes` event params now emit ABI-style hashed topics (`keccak256(payload)`), indexed static tuple/fixed-array params emit ABI-style hashed topics over canonical static in-place encoding, and indexed dynamic arrays whose element type is static (including scalar, static tuple, and static fixed-array elements) now hash canonical in-place element words. Indexed dynamic composites (elements containing dynamic members) still fail with explicit migration guidance.
