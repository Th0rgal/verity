...
- the machine-readable boundary catalog now makes that blocker exact rather than implicit: callers still reach the body theorem through the helper-free `SupportedStmtList` witness, the helper-aware body theorem does not yet consume helper-summary soundness/rank evidence end to end, and the public theorem stack still targets legacy `execIRFunction` / `interpretIR` even though `IRInterpreter.lean` now defines helper-aware compiled-side targets (`execIRFunctionWithInternals`, `interpretIRWithInternals`) that can resolve `IRContract.internalFunctions`. The intended legacy-compatible external-body Yul subset is now formalized directly in `IRInterpreter.lean` as `LegacyCompatibleExternalStmtList`, and `Dispatch.runtimeContractOfFunctions` now has an explicit `internalFunctions = []` lemma documenting the exact helper-free runtime constructor already used by the current theorem stack; the repo now also records that those helper-aware compiled semantics are still encoded as opaque `partial def` executables, so the first conservative-extension theorem still needs a theorem-friendly total or inductive mirror of that target. The remaining compiled-side step is then to prove the conservative-extension lemma from `interpretIR` to `interpretIRWithInternals` first on that runtime-contract target over the subset before those public theorems can be retargeted safely. The compiled-side blocker is tracked in [#1638](https://github.com/Th0rgal/verity/issues/1638)
