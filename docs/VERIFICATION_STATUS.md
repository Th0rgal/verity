...
- Indexed `bytes` event params now emit ABI-style hashed topics (`keccak256(payload)`), indexed static tuple/fixed-array params emit ABI-style hashed topics over canonical static in-place encoding, and indexed scalar-element dynamic arrays (`uint256[]`, `address[]`, `bool[]`, `bytes32[]`) now hash canonical in-place element words. Indexed dynamic composites (arrays of composite elements, tuples/fixed arrays containing dynamic elements) still fail with explicit migration guidance.
