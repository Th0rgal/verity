...
Native-runtime transition status: the current theorem target is still the backend-parameterized Verity statement interpreter. The executable native EVMYulLean path lives in [`EvmYulLeanNativeHarness.lean`](../Compiler/Proofs/YulGeneration/Backends/EvmYulLeanNativeHarness.lean), and the remaining theorem-transition plan is tracked in [`NATIVE_EVMYULLEAN_TRANSITION.md`](NATIVE_EVMYULLEAN_TRANSITION.md). The first instantiated per-case dispatcher bridge for `SimpleStorage` discharges the selector-miss case; the retrieve-hit and store-hit cases remain explicit `simpleStorageNativeRetrieveHitBridge`/`simpleStorageNativeStoreHitBridge` hypotheses on `simpleStorage_endToEnd_native_evmYulLean` because of an unbounded-vs-`UInt256` mismatch between the IR storage type and EVMYulLean's `SharedState`. The clean long-term resolution is the IR storage type refactor tracked in [`IR_STORAGE_UINT256_REFACTOR.md`](IR_STORAGE_UINT256_REFACTOR.md).
