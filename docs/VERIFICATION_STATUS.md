...
- `compileExpr_bridgedSource`: arithmetic/comparison/bit-operation source expressions, storage and storage-array length reads whose compiler field lookup succeeds, calldata/memory/transient reads, and syntactic `keccak256(offset, size)` source expressions in the `BridgedSourceExpr` fragment compile to `BridgedExpr`; `keccak256` remains outside the full source-vs-IR semantic core until memory-slice hashing is modeled by the source evaluator
