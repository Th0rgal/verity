...
- `execYulFuelWithBackend_verity_eq`: the backend-parameterized statement executor recovers existing `legacyExecYulFuel` semantics at `.verity`, giving the next induction a verified executor surface
...
- `emitYul_runtimeCode_evmYulLean_eq_on_bridged_bodies`: emitted runtime-code execution through Verity `legacyExecYulFuel` equals the EVMYulLean backend executor when those embedded function/entrypoint/internal bodies satisfy `BridgedStmts`
- `yulCodegen_preserves_semantics_evmYulLeanBackend_via_reference_oracle`: the lower-level Layer-3 IR→Yul preservation theorem composed with the EVMYulLean backend runtime target; it intentionally remains parameterized by embedded-body `BridgedStmts` witnesses for callers that already have generated-Yul closure proofs. It still composes through `yulCodegen_preserves_semantics_via_reference_oracle` before rewriting the emitted runtime to the EVMYulLean backend executor, so it is not yet the final native source-of-truth architecture.
- `layers2_3_ir_matches_yul_evmYulLeanBackend`: a private transition lemma over the EVMYulLean backend runtime target; raw external function-body `BridgedStmts` witnesses are discharged internally from `SupportedSpec`, static-parameter witnesses, and `BridgedSafeStmts` source-body witnesses, fallback/receive body witnesses are discharged from the `none` hypotheses, and the internal-function-table witness is derived from `ContractWF`. Public EndToEnd correctness targets native dispatcher execution through `interpretIRRuntimeNative`.
...
The backend-parameterized executor now has a proved `.verity = .evmYulLean` theorem for recursive statement targets constrained by `BridgedTarget`, and the generated runtime wrapper is proved to preserve that predicate and to execute equivalently under explicit body-closure hypotheses. Those backend-wrapper EndToEnd lemmas are private transition plumbing. Body closure now has a universal safe-body aggregation theorem for `BridgedSafeStmts`, while the public EndToEnd theorem family targets native dispatcher execution through `interpretIRRuntimeNative` and keeps the external-call/function-table family carved out where needed.

Native-runtime transition status: the public theorem target is native EVMYulLean dispatcher execution. The executable native EVMYulLean path lives in [`EvmYulLeanNativeHarness.lean`](../Compiler/Proofs/YulGeneration/Backends/EvmYulLeanNativeHarness.lean), and the remaining theorem-transition plan is tracked in [`NATIVE_EVMYULLEAN_TRANSITION.md`](NATIVE_EVMYULLEAN_TRANSITION.md). The native EndToEnd seams target `nativeIRRuntimeMatchesIR` directly, with the remaining generated-fragment obligation exposed by the positive dispatcher-exec match theorem family instead of the older reference-oracle/fuel-wrapper wrapper aliases.

Trust boundary (public EndToEnd target): native `EvmYul.Yul.callDispatcher` execution is the public semantic target. The remaining private backend-wrapper lemmas still mention `execYulFuelWithBackend`/`interpretYulRuntimeWithBackend .evmYulLean` as transition evidence for the bridged safe-body fragment, but they are not exported as public compiler-correctness theorems.
