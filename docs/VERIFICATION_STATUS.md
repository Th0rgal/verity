...
- Indexed `bytes` event params now emit ABI-style hashed topics (`keccak256(payload)`), and indexed static tuple/fixed-array params now emit ABI-style hashed topics over their canonical static in-place encoding. Indexed dynamic composites (`T[]`, tuples/fixed arrays containing dynamic elements) still fail with explicit migration guidance.
