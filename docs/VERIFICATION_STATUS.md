...
- the helper-aware source semantics is now proved to be a conservative extension of the current helper-free source semantics on the existing `SupportedSpec` fragment, and the public theorem surface is already anchored to that helper-aware semantics via `SupportedSpec.helperFuel`, so future helper-summary composition can target the same source-level semantics family without another theorem-shape rewrite or trusted-boundary change; `SourceSemantics.lean` now also defines explicit `SupportedFunctionHelperProofs` / `SupportedSpecHelperProofs` wrappers, and `Contract.lean` / `Function.lean` / `Dispatch.lean` now expose matching helper-proof-carrying theorem variants (`*_with_helper_proofs`), so the remaining helper blocker is no longer where summary-soundness evidence should live, but turning that evidence into real compile-preservation on the compiled side
- the machine-readable boundary catalog now makes that blocker exact rather than implicit: callers still reach the body theorem through the helper-free `SupportedStmtList` witness, the helper-aware body theorem does not yet consume helper-summary soundness/rank evidence end to end, and the default theorem path still closes through legacy `execIRFunction` / `interpretIR` even though `IRInterpreter.lean` now defines helper-aware compiled-side targets (`execIRFunctionWithInternals`, `interpretIRWithInternals`) that can resolve `IRContract.internalFunctions` and `Function.lean` / `Dispatch.lean` / `Contract.lean` now expose wrapper theorems that already target those helper-aware compiled semantics once the conservative-extension equalities are supplied. The intended legacy-compatible external-body Yul subset is now formalized directly in `IRInterpreter.lean` as `LegacyCompatibleExternalStmtList`, the helper-free runtime-contract shape is now packaged as `LegacyCompatibleRuntimeContract`, and the exact first retarget theorem is now encoded as `InterpretIRWithInternalsZeroConservativeExtensionGoal`; `IRInterpreter.lean` now also decomposes that theorem into the explicit expr / stmt / stmt-list / function proof surface `InterpretIRWithInternalsZeroConservativeExtensionInterfaces`, factors shared transaction setup through `applyIRTransactionContext`, and encodes the selected-function cut as `InterpretIRWithInternalsZeroConservativeExtensionDispatchGoal` over `LegacyCompatibleRuntimeDispatch`. `IRInterpreter.lean` now also proves `interpretIRWithInternalsZeroConservativeExtensionGoal_of_dispatchGoal`, so the architectural lift from the selected-function theorem back to the contract-level goal is already discharged. The expr / expr-list obligations of that surface are already proved in `IRInterpreter.lean` via `evalIRExprWithInternals_eq_evalIRExpr_of_no_internal`, `evalIRExprsWithInternals_eq_evalIRExprs_of_no_internal`, and `InterpretIRWithInternalsZeroConservativeExtensionExprInterfaces`; `IRInterpreter.lean` now also packages the remaining stmt seam as the named interface `InterpretIRWithInternalsZeroConservativeExtensionStmtSubgoals`, and proves `execIRStmtWithInternals_eq_execIRStmt_of_stmtSubgoals` plus `interpretIRWithInternalsZeroConservativeExtensionInterfaces_of_stmtSubgoals`, so the open compiled-side work is now a compositional Lean object rather than only a prose list of cases; and `IRInterpreter.lean` now also proves `execIRStmtsWithInternals_eq_execIRStmts_of_stmtCompatibility`, `execIRFunctionWithInternals_eq_execIRFunction_of_stmtCompatibility`, `interpretIRWithInternalsZeroConservativeExtensionDispatchGoal_of_stmtCompatibility`, and `interpretIRWithInternalsZeroConservativeExtensionGoal_of_stmtCompatibility`, so stmt-list compatibility, function compatibility, the dispatch-local cut, and the contract-level lift are all reduced to that named stmt-subgoal interface rather than remaining separate blockers. `IRInterpreter.lean` now also classifies dedicated expr-statement builtin cases via `exprStmtUsesDedicatedBuiltinSemantics` and has direct helper-free lemmas for `stop`, `mstore`, `revert`, `return`, and mapping-slot `sstore`, so the remaining compiled-side step is more specific than the raw case list: it is to assemble the residual expr-statement compatibility surface plus nested `if`/`block` transport needed to instantiate `InterpretIRWithInternalsZeroConservativeExtensionStmtSubgoals` end to end. `Dispatch.runtimeContractOfFunctions` now also has a `runtimeContractOfFunctions_legacyCompatible` bridge, and `Dispatch.lean` / `Contract.lean` expose `_goal` variants such as `compile_preserves_semantics_with_helper_proofs_and_helper_ir_goal` so the public theorem seam can consume `InterpretIRWithInternalsZeroConservativeExtensionGoal` plus an explicit legacy-compatibility witness instead of only a raw equality. The helper-aware compiled target remains available as total fuel-indexed helper-aware IR semantics. The compiled-side blocker is tracked in [#1638](https://github.com/Th0rgal/verity/issues/1638)
