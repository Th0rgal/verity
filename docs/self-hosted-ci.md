...
This repository runs `verify.yml` on a dedicated Linux server using two
GitHub Actions self-hosted runners that share a persistent cache root.

## Runner layout

- Runner `1`: labels `self-hosted,linux,x64,verity,build,cpu-8,mem-64g`
- Runner `2`: labels `self-hosted,linux,x64,verity,build,cpu-8,mem-64g`
...
- the `github-runner` user (with passwordless sudo for CI tool installation)
- `/opt/actions-runner`
- `/srv/verity-ci-cache`
...
- Branch and PR caches are isolated by key, but they can still seed from
  `main`.
- The `github-runner` user needs passwordless sudo for installing system
  packages (solc, clang, etc.) during CI runs.
