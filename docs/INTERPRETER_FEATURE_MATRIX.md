...
25/36 builtins have universal bridge agreement proofs between Verity and EVMYulLean evaluation paths (18 fully proven, 7 with sorry-dependent core equivalences). 25 are discharged by universal symbolic lemmas in `Compiler/Proofs/YulGeneration/Backends/EvmYulLeanBridgeLemmas.lean`, and none still require concrete-only regression coverage. The remaining 11 are state-dependent or Verity-specific helpers that remain on the Verity evaluation path.
...
| Builtins (agreement) | 18 | 0 | 7 (`exp`, `sdiv`, `smod`, `slt`, `sgt`, `sar`, `signextend` — sorry-dependent) | 11 (delegated) |
