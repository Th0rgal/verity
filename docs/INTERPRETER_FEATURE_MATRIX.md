...
15/19 builtins have bridge agreement coverage between Verity and EVMYulLean evaluation paths. 9 are discharged by universal symbolic lemmas in `Compiler/Proofs/YulGeneration/Backends/EvmYulLeanBridgeLemmas.lean`, while `and`, `or`, `xor`, `not`, `shl`, and `shr` are currently guarded by concrete regression checks in `Compiler/Proofs/YulGeneration/Backends/EvmYulLeanBridgeTest.lean`. The remaining 4 are state-dependent or Verity-specific helpers that remain on the Verity evaluation path.
