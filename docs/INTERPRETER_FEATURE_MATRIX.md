...
28/36 builtins have universal bridge agreement proofs between Verity and EVMYulLean evaluation paths (23 fully proven, 5 with sorry-dependent core equivalences). 28 are discharged by universal symbolic lemmas in `Compiler/Proofs/YulGeneration/Backends/EvmYulLeanBridgeLemmas.lean`. Of those, 25 are discharged by universal symbolic lemmas for the pure fragment, and none still require concrete-only regression coverage. The remaining 8 are state-dependent or Verity-specific helpers that remain on the Verity evaluation path.
...
| Builtins (agreement) | 23 | 0 | 5 (`exp`, `sdiv`, `smod`, `sar`, `signextend` — sorry-dependent) | 8 (delegated) |
