...
2. Define the exact storage layout and ABI for the base contract. (Storage layout done:
   ...
   - Baseline invariants (ownerCount > 0, threshold > 0, threshold ≤ ownerCount) captured in
     `DumbContracts/Specs/SafeMultisigBase/Invariants.lean`
   - Linked-list owners invariant scaffolded (sentinel + mapping chain) in
     `DumbContracts/Specs/SafeMultisigBase/Invariants.lean` (address encoding left opaque)
3. Implement the base contract in the DumbContracts EDSL.
   - Added minimal EDSL skeletons for `setup` and `execTransaction` with
     input plumbing and baseline checks.
   - Scaffolded `setup` to initialize the owners linked-list mapping using
     sentinel addresses (address encoding remains opaque).
   - Added setup initialization for module sentinel mapping and fallback handler
     storage slot (hashed constants parsed from upstream).
   - Added setup guard to prevent re-initialization (requires stored threshold = 0).
  - Added setup preconditions to disable `to`/`data` delegatecall and payments
    until those paths are modeled.
  - Added execTransaction input guards for valid `operation` (call vs delegatecall)
    and signature length multiple-of-65.
  - Added placeholder `setGuard` and `setModuleGuard` entrypoints restricted to
    self-calls and updating the guard storage slots.
