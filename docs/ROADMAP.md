...
- Builtin bridge: 36 of 36 builtins bridged (25 pure + 11 context/env/storage/helper), with 34 fully proven and 2 sorry'd (smod, sar — complex Int↔UInt256 sign/bit semantics)
...
- **Phase 4 (sorry-dependent recursive statement-target fragment + generated runtime equality)**: `EvmYulLeanRetarget.lean` proves `backends_agree_on_bridged_builtins` and `evalYulExpr_evmYulLean_eq_on_bridged`, establishing that `.verity` and `.evmYulLean` agree for `BridgedExpr` expressions built from bridged builtin calls plus backend-independent `tload`/`mload`, subject to the two sorry-backed smod/sar core equivalences. It also defines `execYulFuelWithBackend`, proves `execYulFuelWithBackend_verity_eq`, proves straight-line/block/if/switch/for statement-fragment helpers, proves `execYulFuelWithBackend_eq_on_bridged_target` for recursive `BridgedTarget` values, proves `emitYul_runtimeCode_bridged`: the emitted runtime wrapper satisfies `BridgedTarget` whenever the IR bodies it embeds satisfy `BridgedStmt`, and proves `emitYul_runtimeCode_evmYulLean_eq_on_bridged_bodies`: emitted runtime-code execution through Verity `execYulFuel` equals the EVMYulLean backend executor under the same body hypotheses. The straight-line fragment covers mapping-slot, literal-slot, and identifier-slot `sstore` statement calls. `EvmYulLeanBodyClosure.lean` proves body-closure increments for scalar calldata parameter prologues (`genParamLoads_scalar_bridged`), static scalar leaf-load helpers for fixed arrays/tuples (`genStaticTypeLoads_calldataload_bridged`), full `genParamLoads` prologues for static scalar parameter lists (`genParamLoads_static_scalar_bridged`), scalar-leaf `letVar`/`assignVar` statement lists (`compileStmtList_binding_leaf_bridged`), pure-expression `letVar`/`assignVar` statement lists (`compileStmtList_pure_binding_bridged`), pure-binding plus unpacked single-slot `setStorage` statement lists (`compileStmtList_storage_fragment_bridged`), external `stop`/`return` terminator statement lists (`compileStmtList_terminator_external_bridged`), internal `return` assignment-plus-`leave` statement lists (`compileStmtList_internal_return_bridged`), plain `Stmt.require` statement lists with bridged failure conditions (`compileStmtList_require_bridged`, using a hypothesis-free `revertWithMessage_bridged` lemma for the generated revert body), mixed external/internal source-body fragments composed from those pieces (`compileStmtList_external_body_fragment_bridged`, `compileStmtList_internal_body_fragment_bridged`), one-layer `Stmt.ite` wrappers around those mixed fragments (`compileStmtList_external_structured_body_fragment_bridged`, `compileStmtList_internal_structured_body_fragment_bridged`), two-level `Stmt.ite` wrappers around one-layer structured branches (`compileStmtList_external_nested_body_fragment_bridged`, `compileStmtList_internal_nested_body_fragment_bridged`), and recursive `Stmt.ite` wrappers around mixed fragments (`compileStmtList_external_recursive_body_fragment_bridged`, `compileStmtList_internal_recursive_body_fragment_bridged`). `EvmYulLeanSourceExprClosure.lean` proves scalar leaf closure (`compileExpr_bridgedSource_leaf`) and pure arithmetic/comparison/bit-operation expression closure (`compileExpr_bridgedSource`) for the `BridgedSourceExpr` fragment. Trust boundary shifts for bridged expressions and recursive bridged statement targets whose builtin dependencies are fully proven; smod/sar uses remain sorry-dependent.
- **Remaining to make retargeting whole-program**: 2 sorry-backed core equivalences (smod/sar — complex Int↔UInt256 sign/bit semantics), extension of compiler-produced body closure beyond scalar/static-scalar calldata parameter prologues and recursive pure-binding/single-slot `setStorage`/`require`/terminator/`Stmt.ite` fragments, plus a Layer-3-composed theorem connecting IR → Yul under `.evmYulLean`
