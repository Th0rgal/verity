...
- **Phase 4 (sorry-dependent recursive statement-target fragment)**: `EvmYulLeanRetarget.lean` proves `backends_agree_on_bridged_builtins` and `evalYulExpr_evmYulLean_eq_on_bridged`, establishing that `.verity` and `.evmYulLean` agree for `BridgedExpr` expressions built from bridged builtin calls plus backend-independent `tload`/`mload`, subject to the two sorry-backed smod/sar core equivalences. It also defines `execYulFuelWithBackend`, proves `execYulFuelWithBackend_verity_eq`, proves straight-line/block/if/switch/for statement-fragment helpers, and now proves `execYulFuelWithBackend_eq_on_bridged_target`, the recursive statement/statement-list target theorem for `BridgedTarget` values whose nested statements satisfy `BridgedStmt`. Trust boundary shifts for bridged expressions and recursive bridged statement targets whose builtin dependencies are fully proven; smod/sar uses remain sorry-dependent.
- **Remaining to make retargeting whole-program**: 2 sorry-backed core equivalences (smod/sar — complex Int↔UInt256 sign/bit semantics), generated-code proof that emitted Yul satisfies `BridgedTarget`/`BridgedStmt`, and a Layer-3-composed theorem connecting IR → Yul under `.evmYulLean`
