...
- `Compiler.Modules.Calls.callWithValue` now provides a standard assumed-status ECM for generic `call{value:v}` over an already prepared calldata slice, with revert-data bubbling. This covers adapter/router patterns that need arbitrary calldata and ETH value transfer while keeping the trust surface visible in reports.
