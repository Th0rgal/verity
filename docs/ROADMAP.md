...
- **Native runtime follow-up (#1737)**: the public theorem target still routes through the backend-parameterized Verity Yul interpreter. `EvmYulLeanNativeHarness.lean` provides the executable native path through `EvmYul.Yul.callDispatcher`; `EndToEnd.lean` keeps the current generic native seams explicitly named as `layer3_contract_preserves_semantics_native_via_reference_oracle_of_evmYulLean_bridge` and `layers2_3_ir_matches_native_evmYulLean_via_reference_oracle_of_evmYulLean_bridge` until the reference-oracle retarget plus EVMYulLean fuel-wrapper dependency is removed. `docs/NATIVE_EVMYULLEAN_TRANSITION.md` tracks the remaining state/result bridge lemmas and theorem flip needed to remove the custom interpreter from the public semantic target.
