...
- `verity-compiler --trust-report <path>` now emits a machine-readable per-contract trust surface covering low-level mechanics usage, linked external assumptions, ECM axioms, explicit `proved` / `assumed` / `unchecked` proof-status buckets for foreign surfaces, and constructor/function-level `usageSites`; `--verbose` mirrors the same boundary in human-readable form, and `--deny-unchecked-dependencies` upgrades unchecked foreign usage from a warning to a fail-closed verification gate.
