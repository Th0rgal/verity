...
- **Phase 4 (sorry-dependent straight-line statement fragment)**: `EvmYulLeanRetarget.lean` proves `backends_agree_on_bridged_builtins` and `evalYulExpr_evmYulLean_eq_on_bridged`, establishing that `.verity` and `.evmYulLean` agree for `BridgedExpr` expressions built from bridged builtin calls plus backend-independent `tload`/`mload`, subject to the two sorry-backed smod/sar core equivalences. It also defines `execYulFuelWithBackend`, proves `execYulFuelWithBackend_verity_eq`, and now proves `execYulFuelWithBackend_eq_on_bridged_straight_stmts` for straight-line `BridgedStraightStmts` lists covering value bindings plus mapping-slot `sstore`, `mstore`, `tstore`, `stop`, `return`, and `revert`. Trust boundary shifts for bridged expressions and straight-line statements whose builtin dependencies are fully proven; smod/sar uses remain sorry-dependent.
- **Remaining to make retargeting whole-program**: 2 sorry-backed core equivalences (smod/sar — complex Int↔UInt256 sign/bit semantics), structured-control-flow induction for `.block`/`.if_`/`.switch`/`.for_`, generated-code proof that emitted Yul satisfies the statement predicate, and a Layer-3-composed theorem connecting IR → Yul under `.evmYulLean`
