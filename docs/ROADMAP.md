...
- **Native runtime follow-up (#1737)**: `EvmYulLeanNativeHarness.lean` provides the executable native path through `EvmYul.Yul.callDispatcher`; `EndToEnd.lean` keeps the public theorem seams on the native result surface (`nativeResultsMatchOn` / `sourceResultMatchesNativeOn`), the supported-compiler `compile_preserves_native_evmYulLean_of_nativeResultsMatchOn` theorem, the canonical dispatcher-exec `compile_preserves_native_evmYulLean_of_generated_dispatcherExec_match` theorem, the helper-free dispatcher lowering wrapper `compile_preserves_native_evmYulLean_of_lowered_generated_dispatcher_noMapping`, the mapping-helper dispatcher lowering wrapper `compile_preserves_native_evmYulLean_of_lowered_generated_dispatcher_mapping`, and the concrete SimpleStorage native theorem, with fuel-indexed `nativeIRRuntimeMatchesIR` seams file-local. The public dispatcher-exec and SimpleStorage native theorems expose source/native obligations directly, package native body/env validation internally, and derive native environment validation from explicit representability/header-use facts. The no-mapping and mapping wrappers accept concrete generated-dispatcher lowering results and construct the full emitted-runtime `lowerRuntimeContractNative` result internally. `docs/NATIVE_EVMYULLEAN_TRANSITION.md` tracks the remaining state/result bridge lemmas and theorem work needed to retire the isolated lower-level retargeting evidence completely.
