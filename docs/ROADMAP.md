...
- **Phase 4 (sorry-dependent expression-level + statement executor)**: `EvmYulLeanRetarget.lean` proves `backends_agree_on_bridged_builtins` and `evalYulExpr_evmYulLean_eq_on_bridged`, establishing that `.verity` and `.evmYulLean` agree for `BridgedExpr` expressions built from bridged builtin calls plus backend-independent `tload`/`mload`, subject to the two sorry-backed smod/sar core equivalences. It now also defines `execYulFuelWithBackend` and proves `execYulFuelWithBackend_verity_eq`, giving the next statement-level induction a backend-parameterized executor that recovers current Verity semantics at `.verity`. Trust boundary shifts for bridged expressions whose builtin dependencies are fully proven; smod/sar expressions remain sorry-dependent.
- **Remaining to make retargeting whole-program**: 2 sorry-backed core equivalences (smod/sar — complex Int↔UInt256 sign/bit semantics), statement-level structural induction over a bridged-statement predicate, generated-code proof that emitted Yul satisfies that predicate, and a Layer-3-composed theorem connecting IR → Yul under `.evmYulLean`
