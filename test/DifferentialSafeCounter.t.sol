 ...
 */
contract DifferentialSafeCounter is YulTestBase, DiffTestConfig, DifferentialTestBase {
    // Compiled contract
    address safeCounter;
    ...
    function testDifferential_EdgeValues() public {
        ...
        for (uint256 i = 0; i < values.length; i++) {
            ...
            vm.store(safeCounter, bytes32(uint256(0)), bytes32(values[i]));
            edslStorage[0] = values[i];

            // Increment from edge value (may revert at MAX)
            bool s1 = executeDifferentialTest("increment", sender);
            assertTrue(s1, "Edge increment test failed");

            // Reset and decrement from edge value (may revert at 0)
            vm.store(safeCounter, bytes32(uint256(0)), bytes32(values[i]));
            edslStorage[0] = values[i];
    ...
    function testDifferential_Random100() public {
        ...
        _runRandomDifferentialTests(startIndex, count, _diffRandomSeed("SafeCounter"));
    }
