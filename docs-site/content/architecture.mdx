...
short-circuiting `bind`, five storage spaces (uint256, address, address-keyed
mapping, uint256-keyed mapping, double mapping), `require`, and a curated set
of `@[simp]` lemmas for proof automation. The `verity_contract` macro lowers
...
satisfied. See [`/compiler-architecture`](/compiler-architecture) for the
narrative and [`/compiler`](/compiler) for the CLI reference.
...
- [Compiler Architecture](/compiler-architecture) — the pipeline in detail.
