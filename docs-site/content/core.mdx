...
description: How the 212-line core works
---

# Core Architecture

The core is 212 lines of Lean 4. It provides types for contract state, storage operations, a `require` guard, and `@[simp]` lemmas for proof automation.
...
Three independent storage spaces: Uint256 slots, Address slots, and Mapping slots. Each is indexed by a natural number (the slot). The state also carries the sender address and the contract's own address.
...
`ContractResult` makes success/failure explicit. Proofs can reason about both paths, and no axioms are needed. The cost is ~130 lines of extra infrastructure (the ContractResult type, custom bind, and simp lemmas).
...
4 out of 7 implementation iterations needed no core changes. The jump from 82 to 212 lines was for verification infrastructure, not contract primitives.
...
[`edsl/DumbContracts/Core.lean`](https://github.com/Th0rgal/dumbcontracts/blob/main/edsl/DumbContracts/Core.lean)
