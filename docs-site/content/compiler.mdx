...
1. Logical specs in `Contracts/<Name>/Spec.lean` are `Prop` statements used for proofs.
...
- **Layer 2** (CompilationModel → IR): **Proved generically, 1 axiom.** The theorem `compile_preserves_semantics` holds for arbitrary supported `CompilationModel`s. It depends on `supported_function_body_correct_from_exact_state` for non-core body simulation (see [AXIOMS.md](https://github.com/Th0rgal/verity/blob/main/AXIOMS.md)).
- **Layer 3** (IR → Yul): **Proved generically.** The dispatch bridge is an explicit theorem hypothesis, not a Lean axiom.
- **Interpreter semantics**: Spec, IR, and Yul semantics defined and linked in Lean.
- See [VERIFICATION_STATUS.md](https://github.com/Th0rgal/verity/blob/main/docs/VERIFICATION_STATUS.md) for proof-status details and [AXIOMS.md](https://github.com/Th0rgal/verity/blob/main/AXIOMS.md) for all documented axioms.
...
### EDSL Implementation
```lean
-- Contracts/SimpleStorage/SimpleStorage.lean
...
Use typed intrinsics (`Expr.mload`, `Expr.keccak256`, `Expr.contractAddress`, `Expr.chainid`, `Stmt.mstore`) instead of `Expr.externalCall` for these opcodes. Builtin names routed through `externalCall` are rejected by interop validation.

#### Partial-proof features

Some expression and statement types compile to correct Yul but are not yet fully modeled in the proof interpreters. Use `--trust-report` to surface the boundary, and the corresponding `--deny-*` flag to fail closed when a contract must stay inside the proved subset.

| Feature | Proof status | `--deny-*` flag | Tracking |
|---------|-------------|-----------------|----------|
| Low-level calls (`call`, `staticcall`) | Codegen only; validated by differential tests | — | [#1332](https://github.com/Th0rgal/verity/issues/1332) |
| `delegatecall` | Codegen only; separate proxy/upgradeability trust boundary | `--deny-proxy-upgradeability` | [#1420](https://github.com/Th0rgal/verity/issues/1420) |
| Memory intrinsics (`mload`, `mstore`, `calldatacopy`, `returndataCopy`, `returndataOptionalBoolAt`) | Partially modeled | `--deny-linear-memory-mechanics` | [#1411](https://github.com/Th0rgal/verity/issues/1411) |
| `rawLog` event emission | Not modeled (compiles correctly) | `--deny-event-emission` | — |
| `keccak256` intrinsic | Axiomatized, not end-to-end modeled | `--deny-axiomatized-primitives` | [#1411](https://github.com/Th0rgal/verity/issues/1411) |

**Local obligations**: Functions or constructors that use raw low-level calls, returndata choreography, or manual memory primitives carry `localObligations` (user-supplied refinement contracts scoped to that boundary). For proof-strict builds, add `--deny-local-obligations` to fail closed on undischarged obligations. Use `--assumption-report` for a flat machine-readable list.

**Upgradeability reviews**: Pair `--layout-compat-report` with `--deny-layout-incompatibility` to compare two contracts' storage layouts and fail closed if the candidate moves or mutates persisted fields.

For Morpho-style `mapping(K => Struct)` / `mapping(K1 => mapping(K2 => Struct))` layouts, declare `FieldType.mappingStruct` / `FieldType.mappingStruct2` and access named members through `Expr.structMember` / `Expr.structMember2`. The `verity_contract` surface exposes matching storage forms:
