...
1. **Spec**
   - Define interface and storage in `DumbContracts/Specs/<Name>/Spec.lean`
   - Add invariants in `DumbContracts/Specs/<Name>/Invariants.lean` (optional)
   - Align function signatures with Solidity types and storage slots

2. **EDSL Implementation**
   - Implement contract logic in `DumbContracts/Examples/<Name>.lean`
   - Reuse helper lemmas for storage updates and mapping reads

3. **Layer 1 Proofs**
   - Prove correctness in `DumbContracts/Specs/<Name>/Proofs.lean`
