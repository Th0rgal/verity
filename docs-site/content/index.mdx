...
**What's `sorry`?** A placeholder in Lean that says "I'll prove this later." It's how incomplete proofs compile. This project currently has `sorry` placeholders in the Layer 2 (Source → IR) proof scripts, which are being repaired after a definition refactor. Layer 3 proofs and all contract-level specification proofs are fully discharged.

**What are axioms?** Statements assumed true without proof. This project currently has 0 documented Lean axioms. The former mapping-slot range axiom has been eliminated via the kernel-computable Keccak engine (see [AXIOMS.md](https://github.com/Th0rgal/verity/blob/main/AXIOMS.md)). Selector hashing is kernel-computable, the former generic Layer 2 body-simulation axiom has been eliminated, and the Layer 3 dispatch bridge is kept as an explicit theorem hypothesis rather than a Lean axiom.
