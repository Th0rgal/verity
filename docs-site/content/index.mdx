...
Eventually, compile the Lean implementation to EVM bytecode. For now, this project demonstrates the specification and verification part of that vision.
...
**Current status**: 212 lines of EDSL core, 7 example contracts, 252 machine-checked proofs. Zero `sorry` (unproven claims), zero axioms (no trusted assumptions).

## The Three-Layer Structure

Each contract has three files:

**1. Specification** (`Specs/SimpleToken/Spec.lean`)
```lean
-- What should mint do?
...
**2. Implementation** (`Examples/SimpleToken.lean`)
```lean
...
**3. Proof** (`Proofs/SimpleToken/Basic.lean`)
```lean
...
| Contract | Theorems | Demonstrates |
|----------|----------|--------------|
| SimpleStorage | 19 | Basic storage read/write, state isolation |
| Counter | 29 | Arithmetic operations, composition |
| Owned | 22 | Access control patterns, ownership transfer |
| SimpleToken | 64 | Token minting/transfer, supply conservation, storage isolation |
| OwnedCounter | 34 | Combining patterns (owned + counter) |
| Ledger | 40 | Balance tracking, deposit/withdraw/transfer, conservation laws |
| SafeCounter | 30 | Overflow/underflow prevention with checked arithmetic |

Plus a math standard library with 14 theorems about safe arithmetic (`safeMul`, `safeDiv`, etc.).

**Total**: 252 proven theorems. See [/verification](/verification) for the complete list.
...
- **[Verification](/verification)** — All 252 theorems listed by contract
- **[Examples](/examples)** — The 7 contracts and what they demonstrate
- **[Core](/core)** — How the 212-line EDSL works
...
- [Build Status](https://github.com/Th0rgal/dumbcontracts/actions) — CI verification runs
