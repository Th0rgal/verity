...
3. `Compiler/Specs.lean`: `CompilationModel` compiler models that generate IR/Yul.
...
def store (value : Uint256) : Contract Unit := setStorage storedData value
def retrieve : Contract Uint256 := getStorage storedData
```

**2. See its proven theorems** — [`Verity/Proofs/SimpleStorage/Basic.lean`](https://github.com/Th0rgal/verity/blob/main/Verity/Proofs/SimpleStorage/Basic.lean) contains theorems like `store_meets_spec` and `retrieve_preserves_state`, each machine-checked by Lean with zero `sorry`. Build just that file to confirm:
```bash
lake build Verity.Proofs.SimpleStorage.Basic    # Verifies in seconds
```

**3. See it compile to EVM bytecode** — after `lake build` completes:
```bash
lake exe verity-compiler    # Outputs Yul to artifacts/yul/
cat artifacts/yul/SimpleStorage.yul
```

This is the core loop: **write a contract in Lean → prove it correct → compile it to EVM bytecode**.
...
Follow the printed instructions to register the contract in `Verity/All.lean` and `Compiler/Specs.lean`, then see the [First Contract tutorial](/first-contract) for a full walkthrough.
