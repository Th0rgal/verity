...
The EVM-shaped state model used by EDSL contracts. Carries five independent
storage spaces (uint256, address, address-keyed mapping, uint256-keyed
mapping, double mapping), the transaction context (`sender`, `thisAddress`,
`msgValue`, `blockTimestamp`), tracked `knownAddresses` per slot for
conservation proofs, and an append-only `events` log.
