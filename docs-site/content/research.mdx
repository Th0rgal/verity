...
- **Layer 3 (IR → Yul)**: In progress (generic equivalence goal scaffolding + Yul fuel‑unfolding lemmas; sequence lemma still blocked on `execIRStmts` opacity; equivalence theorems not yet proven).
