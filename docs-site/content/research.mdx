...
- **Strings are still bounded today**: `String` flows through macro parsing, calldata, `returnBytes`, event payloads, custom errors, and direct parameter `==` / `!=` checks, but Solidity-style string storage/layout, dynamic linked externals, dynamic local aliases, and broader word-style operators still fail fast instead of pretending to be supported (see issue `#1159`).
...
- `Compiler/CompilationModel.lean` — DSL + automatic IR compiler
...
- Test results: see [VERIFICATION_STATUS.md](https://github.com/Th0rgal/verity/blob/main/docs/VERIFICATION_STATUS.md) for the current proof and test status snapshot
...
- Current Foundry and proof status is tracked in [VERIFICATION_STATUS.md](https://github.com/Th0rgal/verity/blob/main/docs/VERIFICATION_STATUS.md)
...
- **Property extraction**: Reduce exclusions. Add property tests contract‑by‑contract. Keep coverage script green.
- **Nested mapping proofs**: Extend storage semantics + proofs for nested mappings to support allowances (`approve`/`transferFrom`).
- **Selector proofs**: Prove selector hashes in Lean (not just CI fixture checks).
- **Documentation**: Keep roadmap updates in sync with each milestone.
