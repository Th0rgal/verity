...
- **Layer 3 (IR → Yul)**: In progress (generic equivalence scaffolding + Yul/IR fuel‑unfolding lemmas + fuel‑bounded IR↔Yul sequence lemma + fuel‑based function equivalence goal + fuel‑bounded function lemma from aligned IR state + explicit fuel‑adequacy goal definitions for IR; full non‑fuel equivalence theorems not yet proven).
...
- **Layer 3 proof equivalence (IR → Yul) — generic path**
  ...
  - Sequence/program equivalence lemma (composition + state mapping) still needs a clean non‑fuel bridge on the IR side (fuel adequacy).
