...
- **Layer 2 (ContractSpec -> IR)**: Per-function preservation theorems complete for all 7 contracts; contract-level dispatch generalization is underway (SimpleStorage + Counter done).
- **Layer 3 (IR -> Yul)**: In progress (core IR/Yul semantics aligned, runtime preservation theorem in place, and smoke tests assert revert preservation across IR and Yul).
...
- [x] Contract-level dispatch preservation exists for SimpleStorage + Counter (selectors wired to transactions).
