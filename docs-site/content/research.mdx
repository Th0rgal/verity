...
- **Layer 3 (IR → Yul)**: In progress (generic equivalence goal scaffolding + Yul fuel‑unfolding lemmas; equivalence theorems not yet proven).
...
- **Layer 3 proof equivalence (IR → Yul) — generic path**
  ...
  - Sequence/program equivalence lemma (composition + state mapping).
