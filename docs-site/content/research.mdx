...
- **Layer 3 (IR → Yul)**: In progress (scaffolding + smoke checks; equivalence theorems not yet proven).
...
- Converted IR↔Yul smoke checks into Lean examples so `Compiler.Proofs.YulGeneration.SmokeTests` builds.
...
- Guard the manifest against drift: keep `test/property_manifest.json` synchronized with proofs via `scripts/check_property_manifest_sync.py`.
  ...
  - New coverage: SafeCounter now includes a property test for `evm_add_eq_of_no_overflow` (EVM `add` vs math addition).
...
- Nested mappings (needed for allowances / ERC20 approve).
  ...
  - Helper added: `encodeNestedMappingSlot` / `decodeNestedMappingSlot` in `Compiler/Proofs/MappingEncoding.lean`.
  - Mapping slot encoding now preserves nested base slots (no modulo truncation), so nested encodings round‑trip.
  - Smoke test added in `Compiler/Proofs/YulGeneration/SmokeTests.lean` to validate nested mapping slot routing in the Yul interpreter.
...
- Add a formalized “bridge” lemma between Yul semantics and EVM, or document the test/verification gap clearly.
  ...
  - **Mitigation today**: Foundry tests provide behavioral confidence and CI now compiles generated Yul with `solc`, but neither closes the formal gap.
