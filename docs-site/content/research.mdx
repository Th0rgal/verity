...
- Current baseline: all 7 contracts have diff-test harnesses in `test/Differential*.t.sol` and random generators in `Compiler/RandomGen.lean`.
...
- Add adversarial patterns: self-transfer (done), max uint values, boundary overflows.
...
5. ~~Self-transfer handling (special-case or identity operation)~~ ✅ **Done**
