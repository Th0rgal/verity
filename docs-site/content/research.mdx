...
- **Layer 3 (IR → Yul)**: In progress (generic equivalence scaffolding + Yul fuel‑unfolding lemmas + fuel‑bounded IR↔Yul sequence lemma + fuel‑based function equivalence goal + fuel‑bounded function lemma from aligned IR state; equivalence theorems not yet proven).
...
- **Layer 3 proof equivalence (IR → Yul) — generic path**
  ...
  - Sequence/program equivalence lemma (composition + state mapping) still needs an `execIRStmts` unfolding lemma plus a fuel‑adequacy bridge.
  - Generic function equivalence theorem (parameterized by function + compiler).
