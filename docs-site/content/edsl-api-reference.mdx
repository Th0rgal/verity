...
`verity_contract` lowers `requireSomeUint` over `safeAdd`, `safeSub`, and `safeMul` to explicit compiled `require` guards before binding the arithmetic result. Bare `add`, `sub`, and `mul` remain wrapping EVM arithmetic.
