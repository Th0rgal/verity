...
The compiler proves Layer 2 preservation automatically for the supported fragment — no manual per-contract bridge proofs are needed. The repository's proof tree currently has `sorry` placeholders in the Layer 2 proof scripts (being repaired after a definition refactor); Layer 3 proofs and contract-level specification proofs are fully discharged. The remaining documented trust surface is tracked in `AXIOMS.md`.
