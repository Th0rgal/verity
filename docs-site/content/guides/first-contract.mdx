...
The compiler proves Layer 2 preservation automatically for the supported fragment — no manual per-contract bridge proofs are needed. The repository's proof tree currently has zero `sorry` placeholders; the remaining documented trust surface is tracked in `AXIOMS.md`.
