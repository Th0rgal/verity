...
`Contracts/Proofs/SemanticBridge.lean` still exists, but it is now a legacy/example wrapper layer rather than the source of Layer 2 correctness. The repository's proof tree currently has zero `sorry` placeholders; the remaining documented trust surface is tracked in `AXIOMS.md`.
