...
**Step 3** — Declare the external function and reference it in your ContractSpec:
```lean
-- In your ContractSpec definition:
externals := [
  { name := "myHash"
...
### 3. Declare externals and add the call to your ContractSpec

In `Compiler/Specs.lean`, first declare the external function in the `externals` field of your ContractSpec, then use `Expr.externalCall` in the function body:
...
> Without the `externals` field on a `ContractSpec`, the linker will skip that contract even if `--link` is passed. No injection occurs and no error is raised.
