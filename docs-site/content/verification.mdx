...
The EDSL has 252 proven theorems across 7 contracts and a math stdlib. The compiler is additionally verified with IR and Yul preservation proofs in `Compiler/Proofs/`. No `sorry`, no axioms. All proofs are checked by `lake build`.

## Compiler Proofs (IR + Yul)

- **Spec semantics**: `Compiler/Proofs/SpecInterpreter.lean`
- **IR generation**: `Compiler/Proofs/IRGeneration/`
...
│   └── Proofs/
│       ├── Basic.lean            # Core proofs (specs met, state preserved)
│       ├── Correctness.lean      # Deeper proofs (composition, revert behavior)
...
### SimpleToken (64 theorems: 36 Basic + 10 Correctness + 9 Supply + 9 Isolation)
...
Transfer proofs require `sender != to` because the implementation writes the recipient balance last, overwriting the sender deduction on self-transfer. Supply conservation uses exact sum equations rather than the `supply_bounds_balances` invariant, which cannot be preserved for lists with duplicate addresses.

**Isolation (9 theorems):**
...
### OwnedCounter (34 theorems: 29 Basic + 5 Correctness)
...
### Ledger (40 theorems: 21 Basic + 6 Correctness + 13 Conservation)
...
### SafeCounter (30 theorems: 22 Basic + 8 Correctness)
