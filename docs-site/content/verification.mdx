...
Per-contract theorem inventory and the structure of the compiler-correctness proofs in `Compiler/Proofs/`. All three layers are complete and checked by `lake build`. For *how* these proofs are built (guard modeling, techniques, limitations) see [Proof Techniques](/proof-techniques). For *what is trusted vs verified*, see [Trust Model](/trust-model).
...
- [Proof Techniques](/proof-techniques) — how the proofs above are constructed (guard modeling, unfolding, list-sum reasoning) and what's currently out of scope.
- [Trust Model](/trust-model) — what each verified theorem rules out, and what assumptions remain.
- [Examples Gallery](/examples) — the contract sources these theorems apply to.
