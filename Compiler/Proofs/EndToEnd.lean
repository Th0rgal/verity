...
theorem layer3_contract_preserves_semantics_evmYulLean_with_function_bridge
  ...
  Compiler.Proofs.YulGeneration.Backends.yulCodegen_preserves_semantics_evmYulLeanFuelWrapperDefaultFuel_of_backend_bridge
...
theorem layer3_contract_preserves_semantics_native_of_evmYulLean_fuel_wrapper_bridge
    (fuel : Nat) (contract : IRContract) (tx : IRTransaction)
...
`layer3_contract_preserves_semantics_native_of_evmYulLean_fuel_wrapper_bridge`
because the generic native path still composes through the EVMYulLean fuel
wrapper and the explicit Layer 3 EVMYulLean fuel-wrapper retarget. -/
...
materialization. -/
theorem layers2_3_ir_matches_native_evmYulLean_of_evmYulLean_fuel_wrapper_bridge
    (fuel : Nat)
    (spec : CompilationModel.CompilationModel) (selectors : List Nat)
...
The proof body intentionally lives in
`layers2_3_ir_matches_native_evmYulLean_of_evmYulLean_fuel_wrapper_bridge`
because the generic native path still reaches native EVMYulLean by comparing
against the EVMYulLean fuel wrapper produced by the explicit Layer 3
fuel-wrapper retarget. -/
