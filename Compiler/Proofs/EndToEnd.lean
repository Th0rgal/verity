/-
  ...
  **EVMYulLean note (native transition)**: the active public native surface in
  ...
  Historical EVMYulLean backend-fuel retargeting lemmas remain isolated in
  `Compiler.Proofs.YulGeneration.Backends.EvmYulLeanRetarget`; this file no
  longer exposes or composes EndToEnd wrappers over that proof-interpreter
  target.
...
/-- Public canonical generated-runtime dispatcher-exec target against IR.

This is the source-level theorem's direct native execution premise: the
...
/-- Public canonical generated-runtime `EvmYul.Yul.callDispatcher` result.

The lowered generated runtime is installed in an EVMYulLean state and run
through `callDispatcher` at canonical generated-runtime fuel, then projected
onto the proof-side observable Yul result surface. -/
...
def nativeGeneratedCallDispatcherMatchesIROn
    ...
    Prop :=
  nativeResultsMatchOn observableSlots (interpretIR contract tx state)
    (nativeGeneratedCallDispatcherResultOf contract tx state observableSlots
      nativeContract)
...
/-- Public native contract shell produced by generated dispatcher lowering
when the runtime includes the native `mappingSlot` helper. -/
def nativeGeneratedDispatcherContractWithMappingOf
    (dispatcher : List EvmYul.Yul.Ast.Stmt) :
    EvmYul.Yul.Ast.YulContract :=
  { dispatcher := .Block dispatcher
    functions :=
  ...
  let initial :=
    Compiler.Proofs.YulGeneration.Backends.Native.initialState
      ...
      (Compiler.Proofs.YulGeneration.Backends.Native.materializedStorageSlots
        (Compiler.runtimeCode contract) observableSlots)
  let nativeResult :=
    match Compiler.Proofs.YulGeneration.Backends.Native.contractDispatcherExecResult
        fuel' nativeContract initial with
    | .error err => .error err
    | .ok finalState =>
        let restored := finalState.reviveJump.overwrite? initial |>.setStore initial
...
private theorem nativeIRRuntimeMatchesIR_of_compiled_generated_dispatcherStmts_project_body_closure_noMapping
    ...
    {selectors : List Nat} {irContract : IRContract}
    {tx : IRTransaction} {state : IRState} {observableSlots : List Nat}
    {dispatcher : List EvmYul.Yul.Ast.Stmt} {nativeYul : YulResult}
    (hCompile : CompilationModel.compile spec selectors = .ok irContract)
    (hSupported : SupportedSpec spec selectors)
    (hNoMapping : irContract.usesMapping = false)
    (hStaticParams : ∀ entry, entry ∈ SourceSemantics.selectorFunctionPairs spec selectors → Compiler.Proofs.YulGeneration.Backends.AllStaticScalarParams entry.1.params)
...
private theorem nativeIRRuntimeMatchesIR_of_compiled_generated_lowered_runtime_dispatcherStmts_positive_body_closure_noMapping_canonicalFuel
    ...
    nativeIRRuntimeMatchesIR
      (nativeRuntimeFuel irContract) irContract tx state observableSlots :=
  nativeIRRuntimeMatchesIR_of_compiled_generated_lowered_runtime_dispatcherStmts_positive_body_closure_noMapping
    hCompile hSupported hNoMapping hStaticParams hSafeBodies hLower hEnv
    hMatch
...
private theorem nativeIRRuntimeMatchesIR_of_compiled_generated_lowered_runtime_dispatcherStmts_project_body_closure_noMapping_canonicalFuel
    ...
    nativeIRRuntimeMatchesIR
      (nativeRuntimeFuel irContract) irContract tx state observableSlots :=
  nativeIRRuntimeMatchesIR_of_compiled_generated_lowered_runtime_dispatcherStmts_project_body_closure_noMapping
    hCompile hSupported hNoMapping hStaticParams hSafeBodies hLower hEnv
    hProject hMatch
...
private theorem nativeIRRuntimeMatchesIR_of_compiled_generated_lowered_runtime_dispatcherStmts_positive_body_closure_mapping_reserved_canonicalFuel
    ...
    nativeIRRuntimeMatchesIR
      (nativeRuntimeFuel irContract) irContract tx state observableSlots :=
  nativeIRRuntimeMatchesIR_of_compiled_generated_lowered_runtime_dispatcherStmts_positive_body_closure_mapping_reserved
    hCompile hSupported hMapping hStaticParams hSafeBodies hLower hEnv hMatch
...
private theorem nativeIRRuntimeMatchesIR_of_compiled_generated_lowered_runtime_dispatcherStmts_project_body_closure_mapping_reserved_canonicalFuel
    ...
    nativeIRRuntimeMatchesIR
      (nativeRuntimeFuel irContract) irContract tx state observableSlots :=
  nativeIRRuntimeMatchesIR_of_compiled_generated_lowered_runtime_dispatcherStmts_project_body_closure_mapping_reserved
...
private theorem nativeIRRuntimeMatchesIR_of_compiled_generated_dispatcherStmts_positive_body_closure_mapping_reserved_ofIR_environment
    ...
    (hMatch : nativeDispatcherExecMatchesIRPositive fuel' irContract tx state
      observableSlots (nativeContractOfDispatcherWithMapping dispatcher)) :
    nativeIRRuntimeMatchesIR (Nat.succ fuel') irContract tx state observableSlots :=
  nativeIRRuntimeMatchesIR_of_compiled_generated_dispatcherStmts_positive_body_closure_mapping_reserved
    hCompile hSupported hMapping hStaticParams hSafeBodies hLowerDispatcher
    (Compiler.Proofs.YulGeneration.Backends.Native.validateNativeRuntimeEnvironment_ofIR_representableEnvironment
      (Compiler.emitYul irContract).runtimeCode tx hChainId hBlobBaseFee
      hNoHeader)
...
private theorem nativeIRRuntimeMatchesIR_of_compiled_generated_dispatcherStmts_project_body_closure_mapping_reserved_ofIR_environment
    ...
    (hMatch :
      nativeResultsMatchOn observableSlots (interpretIR irContract tx state)
        (.ok nativeYul)) :
    nativeIRRuntimeMatchesIR (Nat.succ fuel') irContract tx state observableSlots :=
  nativeIRRuntimeMatchesIR_of_compiled_generated_dispatcherStmts_project_body_closure_mapping_reserved
    hCompile hSupported hMapping hStaticParams hSafeBodies hLowerDispatcher
    (Compiler.Proofs.YulGeneration.Backends.Native.validateNativeRuntimeEnvironment_ofIR_representableEnvironment
      (Compiler.emitYul irContract).runtimeCode tx hChainId hBlobBaseFee
      hNoHeader)
...
private theorem nativeIRRuntimeMatchesIR_of_compiled_generated_dispatcherStmts_positive_body_closure_noMapping_canonicalFuel
    ...
    (hEnv : Compiler.Proofs.YulGeneration.Backends.Native.validateNativeRuntimeEnvironment
      (Compiler.emitYul irContract).runtimeCode (YulTransaction.ofIR tx) = .ok ())
    (hMatch : nativeDispatcherExecMatchesIRPositive
      (nativeRuntimeDispatcherFuel irContract) irContract tx state
      observableSlots (nativeContractOfDispatcher dispatcher)) :
    nativeIRRuntimeMatchesIR
      (nativeRuntimeFuel irContract) irContract tx state observableSlots :=
...
private theorem nativeIRRuntimeMatchesIR_of_compiled_generated_dispatcherStmts_project_body_closure_mapping_reserved_canonicalFuel
    ...
    {irContract : IRContract} {tx : IRTransaction} {state : IRState}
    {observableSlots : List Nat} {dispatcher : List EvmYul.Yul.Ast.Stmt}
    {nextSwitchId : Nat} {nativeYul : YulResult}
    (hCompile : CompilationModel.compile spec selectors = .ok irContract)
    (hSupported : SupportedSpec spec selectors)
    (hMapping : irContract.usesMapping = true)
    (hStaticParams : ∀ entry, entry ∈ SourceSemantics.selectorFunctionPairs spec selectors → Compiler.Proofs.YulGeneration.Backends.AllStaticScalarParams entry.1.params)
...
private theorem nativeIRRuntimeMatchesIR_of_compiled_generated_dispatcherStmts_positive_body_closure_noMapping_ofIR_environment_canonicalFuel
  ...
  nativeIRRuntimeMatchesIR_of_compiled_generated_dispatcherStmts_positive_body_closure_noMapping_ofIR_environment
    hCompile hSupported hNoMapping hStaticParams hSafeBodies hLowerDispatcher
    hChainId hBlobBaseFee hNoHeader hMatch
...
private theorem nativeIRRuntimeMatchesIR_of_compiled_generated_dispatcherStmts_positive_body_closure_noMapping_ofIR_globalDefaults_canonicalFuel
    ...
    nativeIRRuntimeMatchesIR
      (nativeRuntimeFuel irContract) irContract tx state observableSlots :=
  nativeIRRuntimeMatchesIR_of_compiled_generated_dispatcherStmts_positive_body_closure_noMapping_ofIR_globalDefaults
    hCompile hSupported hNoMapping hStaticParams hSafeBodies hLowerDispatcher
    hChainId hBlobBaseFee hNoHeader hMatch
...
private theorem nativeIRRuntimeMatchesIR_of_compiled_generated_dispatcherStmts_project_body_closure_noMapping_ofIR_environment_canonicalFuel
    ...
    nativeIRRuntimeMatchesIR
      (nativeRuntimeFuel irContract) irContract tx state observableSlots :=
  nativeIRRuntimeMatchesIR_of_compiled_generated_dispatcherStmts_project_body_closure_noMapping_ofIR_environment
    hCompile hSupported hNoMapping hStaticParams hSafeBodies hLowerDispatcher
...
private theorem nativeIRRuntimeMatchesIR_of_compiled_generated_dispatcherStmts_project_body_closure_noMapping_ofIR_globalDefaults_canonicalFuel
    ...
    nativeIRRuntimeMatchesIR
      (nativeRuntimeFuel irContract) irContract tx state observableSlots :=
  nativeIRRuntimeMatchesIR_of_compiled_generated_dispatcherStmts_project_body_closure_noMapping_ofIR_globalDefaults
    hCompile hSupported hNoMapping hStaticParams hSafeBodies hLowerDispatcher
    hChainId hBlobBaseFee hNoHeader hProject hMatch
...
private theorem nativeIRRuntimeMatchesIR_of_compiled_generated_dispatcherStmts_positive_body_closure_mapping_reserved_ofIR_environment_canonicalFuel
    ...
    nativeIRRuntimeMatchesIR
      (nativeRuntimeFuel irContract) irContract tx state observableSlots :=
  nativeIRRuntimeMatchesIR_of_compiled_generated_dispatcherStmts_positive_body_closure_mapping_reserved_ofIR_environment
    hCompile hSupported hMapping hStaticParams hSafeBodies hLowerDispatcher
    hChainId hBlobBaseFee hNoHeader hMatch
...
private theorem nativeIRRuntimeMatchesIR_of_compiled_generated_dispatcherStmts_positive_body_closure_mapping_reserved_ofIR_globalDefaults_canonicalFuel
    ...
    nativeIRRuntimeMatchesIR
      (nativeRuntimeFuel irContract) irContract tx state observableSlots :=
  nativeIRRuntimeMatchesIR_of_compiled_generated_dispatcherStmts_positive_body_closure_mapping_reserved_ofIR_globalDefaults
    hCompile hSupported hMapping hStaticParams hSafeBodies hLowerDispatcher
    hChainId hBlobBaseFee hNoHeader hMatch
...
private theorem nativeIRRuntimeMatchesIR_of_compiled_generated_dispatcherStmts_project_body_closure_mapping_reserved_ofIR_environment_canonicalFuel
    ...
    nativeIRRuntimeMatchesIR
      (nativeRuntimeFuel irContract) irContract tx state observableSlots :=
  nativeIRRuntimeMatchesIR_of_compiled_generated_dispatcherStmts_project_body_closure_mapping_reserved_ofIR_environment
    hCompile hSupported hMapping hStaticParams hSafeBodies hLowerDispatcher
    hChainId hBlobBaseFee hNoHeader hProject hMatch
...
private theorem nativeIRRuntimeMatchesIR_of_compiled_generated_dispatcherStmts_project_body_closure_mapping_reserved_ofIR_globalDefaults_canonicalFuel
    ...
    nativeIRRuntimeMatchesIR
      (nativeRuntimeFuel irContract) irContract tx state observableSlots :=
  nativeIRRuntimeMatchesIR_of_compiled_generated_dispatcherStmts_project_body_closure_mapping_reserved_ofIR_globalDefaults
    hCompile hSupported hMapping hStaticParams hSafeBodies hLowerDispatcher
    hChainId hBlobBaseFee hNoHeader hProject hMatch
...
private theorem nativeIRRuntimeMatchesIR_of_compiled_generated_lowered_dispatcherExec_project_body_closure_ofIR_environment_canonicalFuel
    ...
    nativeIRRuntimeMatchesIR
      (nativeRuntimeFuel irContract) irContract tx state observableSlots :=
  nativeIRRuntimeMatchesIR_of_compiled_generated_lowered_dispatcherExec_project_body_closure_canonicalFuel
    hCompile hSupported hStaticParams hSafeBodies hLower
    (Compiler.Proofs.YulGeneration.Backends.Native.validateNativeRuntimeEnvironment_ofIR_representableEnvironment
      (Compiler.emitYul irContract).runtimeCode tx hChainId hBlobBaseFee
      hNoHeader)
...
private theorem layer3_contract_preserves_semantics_native_of_compiled_generated_dispatcherExec_positive_external_bodies_match_canonicalFuel
  ...
  layer3_contract_preserves_semantics_native_of_compiled_generated_dispatcherExec_positive_external_bodies_match
    ...
    initialState observableSlots nativeContract hCompile hSupported
...
private theorem layer3_contract_preserves_semantics_native_of_compiled_generated_dispatcherExec_project_external_bodies_match
    ...
    (hEnv : Compiler.Proofs.YulGeneration.Backends.Native.validateNativeRuntimeEnvironment
      (Compiler.emitYul irContract).runtimeCode (YulTransaction.ofIR tx) = .ok ())
    (hProject : nativeProjectedDispatcherResultEq fuel' irContract tx initialState
      observableSlots nativeContract nativeYul)
    (hMatch :
      nativeResultsMatchOn observableSlots (interpretIR irContract tx initialState)
        (.ok nativeYul)) :
...
private theorem layer3_contract_preserves_semantics_native_of_compiled_generated_dispatcherExec_project_external_bodies_match_canonicalFuel
  ...
  layer3_contract_preserves_semantics_native_of_compiled_generated_dispatcherExec_project_external_bodies_match
    (nativeRuntimeDispatcherFuel irContract) spec selectors irContract tx
    initialState observableSlots nativeContract nativeYul hCompile hSupported
    hExternalBodies hLower hEnv hProject hMatch
...
private theorem layer3_contract_preserves_semantics_native_of_compiled_generated_dispatcherExec_positive_body_closure_canonicalFuel
  ...
  layer3_contract_preserves_semantics_native_of_compiled_generated_dispatcherExec_positive_body_closure
    (nativeRuntimeDispatcherFuel irContract) spec selectors irContract tx
    initialState observableSlots nativeContract hCompile hSupported
    hStaticParams hSafeBodies hLower hEnv hNativeDispatcherExec
...
private theorem layer3_contract_preserves_semantics_native_of_compiled_generated_dispatcherExec_project_body_closure
    ...
    nativeResultsMatchOn observableSlots (interpretIR irContract tx initialState)
      (Compiler.Proofs.YulGeneration.Backends.Native.interpretIRRuntimeNative
        (Nat.succ fuel') irContract tx initialState observableSlots) :=
  layer3_contract_preserves_semantics_native_of_compiled_generated_dispatcherExec_project_external_bodies_match
    fuel' spec selectors irContract tx initialState observableSlots nativeContract nativeYul
    hCompile hSupported
    (generatedRuntimeExternalBodiesHaveNoFuncDefs_of_compile_ok_safe
      hCompile hSupported hStaticParams hSafeBodies)
    hLower hEnv hProject hMatch
...
private theorem layer3_contract_preserves_semantics_native_of_compiled_generated_dispatcherExec_project_body_closure_canonicalFuel
  ...
  layer3_contract_preserves_semantics_native_of_compiled_generated_dispatcherExec_project_body_closure
    (nativeRuntimeDispatcherFuel irContract) spec selectors irContract tx
    initialState observableSlots nativeContract nativeYul hCompile hSupported
    hStaticParams hSafeBodies hLower hEnv hProject hMatch
...
private theorem layer3_contract_preserves_semantics_native_of_compiled_generated_dispatcherStmts_positive_body_closure_noMapping_ofIR_environment_canonicalFuel
    ...
    nativeResultsMatchOn observableSlots (interpretIR irContract tx initialState)
      (Compiler.Proofs.YulGeneration.Backends.Native.interpretIRRuntimeNative
        (nativeRuntimeFuel irContract) irContract tx initialState observableSlots) :=
  layer3_contract_preserves_semantics_native_of_compiled_generated_dispatcherStmts_positive_body_closure_noMapping_ofIR_environment
    (nativeRuntimeDispatcherFuel irContract) spec selectors irContract tx
    initialState observableSlots dispatcher hCompile hSupported hNoMapping
    hStaticParams hSafeBodies hLowerDispatcher hChainId hBlobBaseFee hNoHeader
    hNativeDispatcherExec
...
private theorem layer3_contract_preserves_semantics_native_of_compiled_generated_dispatcherStmts_positive_body_closure_noMapping_ofIR_globalDefaults_canonicalFuel
    ...
    nativeResultsMatchOn observableSlots (interpretIR irContract tx initialState)
      (Compiler.Proofs.YulGeneration.Backends.Native.interpretIRRuntimeNative
        (nativeRuntimeFuel irContract) irContract tx initialState observableSlots) :=
  layer3_contract_preserves_semantics_native_of_compiled_generated_dispatcherStmts_positive_body_closure_noMapping_ofIR_globalDefaults
    (nativeRuntimeDispatcherFuel irContract) spec selectors irContract tx
    initialState observableSlots dispatcher hCompile hSupported hNoMapping
    hStaticParams hSafeBodies hLowerDispatcher hChainId hBlobBaseFee hNoHeader
    hNativeDispatcherExec
...
private theorem layer3_contract_preserves_semantics_native_of_compiled_generated_dispatcherStmts_project_body_closure_noMapping_ofIR_environment_canonicalFuel
    ...
    nativeResultsMatchOn observableSlots (interpretIR irContract tx initialState)
      (Compiler.Proofs.YulGeneration.Backends.Native.interpretIRRuntimeNative
        (nativeRuntimeFuel irContract) irContract tx initialState observableSlots) :=
  layer3_contract_preserves_semantics_native_of_compiled_generated_dispatcherStmts_project_body_closure_noMapping_ofIR_environment
    (nativeRuntimeDispatcherFuel irContract) spec selectors irContract tx
    initialState observableSlots dispatcher nativeYul hCompile hSupported
...
private theorem layer3_contract_preserves_semantics_native_of_compiled_generated_dispatcherStmts_project_body_closure_noMapping_ofIR_globalDefaults_canonicalFuel
    ...
    nativeResultsMatchOn observableSlots (interpretIR irContract tx initialState)
      (Compiler.Proofs.YulGeneration.Backends.Native.interpretIRRuntimeNative
        (nativeRuntimeFuel irContract) irContract tx initialState observableSlots) :=
  layer3_contract_preserves_semantics_native_of_compiled_generated_dispatcherStmts_project_body_closure_noMapping_ofIR_globalDefaults
    (nativeRuntimeDispatcherFuel irContract) spec selectors irContract tx
    initialState observableSlots dispatcher nativeYul hCompile hSupported
    hNoMapping hStaticParams hSafeBodies hLowerDispatcher hChainId
    hBlobBaseFee hNoHeader hProject hMatch
...
private theorem layer3_contract_preserves_semantics_native_of_compiled_generated_dispatcherStmts_positive_body_closure_mapping_reserved_ofIR_environment_canonicalFuel
    ...
    nativeResultsMatchOn observableSlots (interpretIR irContract tx initialState)
      (Compiler.Proofs.YulGeneration.Backends.Native.interpretIRRuntimeNative
        (nativeRuntimeFuel irContract) irContract tx initialState observableSlots) :=
  layer3_contract_preserves_semantics_native_of_compiled_generated_dispatcherStmts_positive_body_closure_mapping_reserved_ofIR_environment
    (nativeRuntimeDispatcherFuel irContract) spec selectors irContract tx
    initialState observableSlots dispatcher nextSwitchId hCompile hSupported
...
private theorem layer3_contract_preserves_semantics_native_of_compiled_generated_dispatcherStmts_positive_body_closure_mapping_reserved_ofIR_globalDefaults_canonicalFuel
    ...
    nativeResultsMatchOn observableSlots (interpretIR irContract tx initialState)
      (Compiler.Proofs.YulGeneration.Backends.Native.interpretIRRuntimeNative
        (nativeRuntimeFuel irContract) irContract tx initialState observableSlots) :=
  layer3_contract_preserves_semantics_native_of_compiled_generated_dispatcherStmts_positive_body_closure_mapping_reserved_ofIR_globalDefaults
    (nativeRuntimeDispatcherFuel irContract) spec selectors irContract tx
    initialState observableSlots dispatcher nextSwitchId hCompile hSupported
    hMapping hStaticParams hSafeBodies hLowerDispatcher hChainId
    hBlobBaseFee hNoHeader hNativeDispatcherExec
...
private theorem layer3_contract_preserves_semantics_native_of_compiled_generated_dispatcherStmts_project_body_closure_mapping_reserved_ofIR_environment_canonicalFuel
    ...
    nativeResultsMatchOn observableSlots (interpretIR irContract tx initialState)
      (Compiler.Proofs.YulGeneration.Backends.Native.interpretIRRuntimeNative
        (nativeRuntimeFuel irContract) irContract tx initialState observableSlots) :=
  layer3_contract_preserves_semantics_native_of_compiled_generated_dispatcherStmts_project_body_closure_mapping_reserved_ofIR_environment
    (nativeRuntimeDispatcherFuel irContract) spec selectors irContract tx
    initialState observableSlots dispatcher nextSwitchId nativeYul hCompile
...
private theorem layer3_contract_preserves_semantics_native_of_compiled_generated_dispatcherStmts_project_body_closure_mapping_reserved_ofIR_globalDefaults_canonicalFuel
    ...
    nativeResultsMatchOn observableSlots (interpretIR irContract tx initialState)
      (Compiler.Proofs.YulGeneration.Backends.Native.interpretIRRuntimeNative
        (nativeRuntimeFuel irContract) irContract tx initialState observableSlots) :=
  layer3_contract_preserves_semantics_native_of_compiled_generated_dispatcherStmts_project_body_closure_mapping_reserved_ofIR_globalDefaults
    (nativeRuntimeDispatcherFuel irContract) spec selectors irContract tx
    initialState observableSlots dispatcher nextSwitchId nativeYul hCompile
    hSupported hMapping hStaticParams hSafeBodies hLowerDispatcher hChainId
    hBlobBaseFee hNoHeader hProject hMatch
...
private theorem layer3_contract_preserves_semantics_native_of_compiled_generated_lowered_runtime_dispatcherStmts_positive_body_closure_mapping_reserved
    ...
    (hEnv : Compiler.Proofs.YulGeneration.Backends.Native.validateNativeRuntimeEnvironment
      (Compiler.emitYul irContract).runtimeCode (YulTransaction.ofIR tx) = .ok ())
    (hNativeDispatcherExec : ∀ (dispatcher : List EvmYul.Yul.Ast.Stmt) (nextSwitchId : Nat),
      Compiler.Proofs.YulGeneration.Backends.lowerStmtsNativeWithSwitchIds
          (Compiler.Proofs.YulGeneration.Backends.yulStmtsIdentifierNames
            (Compiler.emitYul irContract).runtimeCode)
          0 [Compiler.CodegenCommon.buildSwitch irContract.functions none none] =
...
private theorem layer3_contract_preserves_semantics_native_of_compiled_generated_lowered_runtime_dispatcherStmts_positive_body_closure_noMapping_canonicalFuel
    ...
    nativeResultsMatchOn observableSlots (interpretIR irContract tx initialState)
      (Compiler.Proofs.YulGeneration.Backends.Native.interpretIRRuntimeNative
        (nativeRuntimeFuel irContract) irContract tx initialState observableSlots) :=
  layer3_contract_preserves_semantics_native_of_compiled_generated_lowered_runtime_dispatcherStmts_positive_body_closure_noMapping
    (nativeRuntimeDispatcherFuel irContract) spec selectors irContract tx
    initialState observableSlots nativeContract hCompile hSupported hNoMapping
    hStaticParams hSafeBodies hLower hEnv hNativeDispatcherExec
...
private theorem layer3_contract_preserves_semantics_native_of_compiled_generated_lowered_runtime_dispatcherStmts_project_body_closure_noMapping_canonicalFuel
    ...
    nativeResultsMatchOn observableSlots (interpretIR irContract tx initialState)
      (Compiler.Proofs.YulGeneration.Backends.Native.interpretIRRuntimeNative
        (nativeRuntimeFuel irContract) irContract tx initialState observableSlots) :=
  layer3_contract_preserves_semantics_native_of_compiled_generated_lowered_runtime_dispatcherStmts_project_body_closure_noMapping
    (nativeRuntimeDispatcherFuel irContract) spec selectors irContract tx
    initialState observableSlots nativeContract nativeYul hCompile hSupported
    hNoMapping hStaticParams hSafeBodies hLower hEnv hProject hMatch
...
private theorem layer3_contract_preserves_semantics_native_of_compiled_generated_lowered_runtime_dispatcherStmts_positive_body_closure_mapping_reserved_canonicalFuel
    ...
    nativeResultsMatchOn observableSlots (interpretIR irContract tx initialState)
      (Compiler.Proofs.YulGeneration.Backends.Native.interpretIRRuntimeNative
        (nativeRuntimeFuel irContract) irContract tx initialState observableSlots) :=
  layer3_contract_preserves_semantics_native_of_compiled_generated_lowered_runtime_dispatcherStmts_positive_body_closure_mapping_reserved
    (nativeRuntimeDispatcherFuel irContract) spec selectors irContract tx
    initialState observableSlots nativeContract hCompile hSupported hMapping
    hStaticParams hSafeBodies hLower hEnv hNativeDispatcherExec
...
private theorem layer3_contract_preserves_semantics_native_of_compiled_generated_lowered_runtime_dispatcherStmts_project_body_closure_mapping_reserved_canonicalFuel
    ...
    nativeResultsMatchOn observableSlots (interpretIR irContract tx initialState)
      (Compiler.Proofs.YulGeneration.Backends.Native.interpretIRRuntimeNative
        (nativeRuntimeFuel irContract) irContract tx initialState observableSlots) :=
  layer3_contract_preserves_semantics_native_of_compiled_generated_lowered_runtime_dispatcherStmts_project_body_closure_mapping_reserved
    (nativeRuntimeDispatcherFuel irContract) spec selectors irContract tx
    initialState observableSlots nativeContract nativeYul hCompile hSupported
    hMapping hStaticParams hSafeBodies hLower hEnv hProject hMatch
...
private theorem compile_preserves_native_evmYulLean_of_generated_dispatcherExec_match
  ...
  have hNativeDispatcherExec' :
      nativeDispatcherExecMatchesIRPositive
        (nativeRuntimeDispatcherFuel irContract) irContract tx
        (FunctionBody.initialIRStateForTx model tx initialWorld)
        observableSlots nativeContract := by
    simpa [
  ...
  have hLayer3 :
      nativeResultsMatchOn observableSlots
        (interpretIR irContract tx
          (FunctionBody.initialIRStateForTx model tx initialWorld))
        (Compiler.Proofs.YulGeneration.Backends.Native.interpretIRRuntimeNative
          (nativeRuntimeFuel irContract) irContract tx
          (FunctionBody.initialIRStateForTx model tx initialWorld)
          observableSlots) :=
  ...
  have hSourceIR :
      Compiler.Proofs.IRGeneration.FunctionBody.sourceResultMatchesIRResult
        ...
        (interpretIR irContract tx
          (FunctionBody.initialIRStateForTx model tx initialWorld)) :=
    Compiler.Proofs.IRGeneration.Contract.compile_preserves_semantics
      model selectors hSupported irContract tx initialWorld htxNormalized
...
private theorem compile_preserves_native_evmYulLean_of_generated_callDispatcher_match
  ...
  unfold nativeGeneratedCallDispatcherMatchesIROn nativeGeneratedCallDispatcherResultOf at hNativeCallDispatcher
    ...
    sourceResultMatchesNativeOn_of_sourceResultMatchesIRResult_of_nativeResultsMatchOn
      (hSourceIR :=
        Compiler.Proofs.IRGeneration.Contract.compile_preserves_semantics
          model selectors hSupported irContract tx initialWorld
          htxNormalized hcalldataSizeFits hcompile)
      (hNativeIR := hNativeCallDispatcher)
...
private theorem compile_preserves_native_evmYulLean_of_lowered_generated_dispatcher_noMapping
  ...
  · exact hBlobBaseFee
  · exact hNoHeader
  · exact hNativeDispatcherExec

/-- Helper-free generated dispatcher wrapper over concrete dispatcher lowering,
with a public `EvmYul.Yul.callDispatcher` native premise. -/
...
/-- Mapping-helper generated dispatcher wrapper over concrete dispatcher
lowering, with a public `EvmYul.Yul.callDispatcher` native premise. -/
...
private theorem layers2_3_ir_matches_native_evmYulLean_of_generated_dispatcherExec_positive_body_closure_ofIR_environment_canonicalFuel
  ...
  layers2_3_ir_matches_native_evmYulLean_of_generated_dispatcherExec_positive_body_closure_canonicalFuel
    spec selectors irContract tx initialState observableSlots nativeContract
    hCompile hSupported hStaticParams hSafeBodies hLower
    (Compiler.Proofs.YulGeneration.Backends.Native.validateNativeRuntimeEnvironment_ofIR_representableEnvironment
      (Compiler.emitYul irContract).runtimeCode tx hChainId hBlobBaseFee
      hNoHeader)
...
private theorem layers2_3_ir_matches_native_evmYulLean_of_generated_dispatcherStmts_positive_body_closure_noMapping_ofIR_environment_canonicalFuel
    ...
    nativeResultsMatchOn observableSlots (interpretIR irContract tx initialState)
      (Compiler.Proofs.YulGeneration.Backends.Native.interpretIRRuntimeNative
        (nativeRuntimeFuel irContract) irContract tx initialState observableSlots) :=
  layers2_3_ir_matches_native_evmYulLean_of_generated_dispatcherStmts_positive_body_closure_noMapping_ofIR_environment
    (nativeRuntimeDispatcherFuel irContract) spec selectors irContract tx
    initialState observableSlots dispatcher hCompile hSupported hNoMapping
    hStaticParams hSafeBodies hLowerDispatcher hChainId hBlobBaseFee hNoHeader
    hNativeDispatcherExec
...
private theorem layers2_3_ir_matches_native_evmYulLean_of_generated_dispatcherStmts_positive_body_closure_noMapping_ofIR_globalDefaults_canonicalFuel
    ...
    nativeResultsMatchOn observableSlots (interpretIR irContract tx initialState)
      (Compiler.Proofs.YulGeneration.Backends.Native.interpretIRRuntimeNative
        (nativeRuntimeFuel irContract) irContract tx initialState observableSlots) :=
  layers2_3_ir_matches_native_evmYulLean_of_generated_dispatcherStmts_positive_body_closure_noMapping_ofIR_globalDefaults
    (nativeRuntimeDispatcherFuel irContract) spec selectors irContract tx
    initialState observableSlots dispatcher hCompile hSupported hNoMapping
    hStaticParams hSafeBodies hLowerDispatcher hChainId hBlobBaseFee hNoHeader
    hNativeDispatcherExec
...
private theorem layers2_3_ir_matches_native_evmYulLean_of_generated_dispatcherExec_project_body_closure_ofIR_environment_canonicalFuel
  ...
  layers2_3_ir_matches_native_evmYulLean_of_generated_dispatcherExec_project_body_closure_canonicalFuel
    spec selectors irContract tx initialState observableSlots nativeContract
    nativeYul hCompile hSupported hStaticParams hSafeBodies hLower
    (Compiler.Proofs.YulGeneration.Backends.Native.validateNativeRuntimeEnvironment_ofIR_representableEnvironment
      (Compiler.emitYul irContract).runtimeCode tx hChainId hBlobBaseFee
      hNoHeader)
    hProject hMatch
...
/-- Supported no-mapping compiler-produced projected-result
...
private theorem layers2_3_ir_matches_native_evmYulLean_of_generated_dispatcherStmts_project_body_closure_noMapping_ofIR_environment_canonicalFuel
    ...
    nativeResultsMatchOn observableSlots (interpretIR irContract tx initialState)
      (Compiler.Proofs.YulGeneration.Backends.Native.interpretIRRuntimeNative
        (nativeRuntimeFuel irContract) irContract tx initialState observableSlots) :=
  layers2_3_ir_matches_native_evmYulLean_of_generated_dispatcherStmts_project_body_closure_noMapping_ofIR_environment
    (nativeRuntimeDispatcherFuel irContract) spec selectors irContract tx
    initialState observableSlots dispatcher nativeYul hCompile hSupported
    hNoMapping hStaticParams hSafeBodies hLowerDispatcher hChainId
    hBlobBaseFee hNoHeader hProject hMatch
...
private theorem layers2_3_ir_matches_native_evmYulLean_of_generated_dispatcherStmts_project_body_closure_noMapping_ofIR_globalDefaults_canonicalFuel
    ...
    nativeResultsMatchOn observableSlots (interpretIR irContract tx initialState)
      (Compiler.Proofs.YulGeneration.Backends.Native.interpretIRRuntimeNative
        (nativeRuntimeFuel irContract) irContract tx initialState observableSlots) :=
  layers2_3_ir_matches_native_evmYulLean_of_generated_dispatcherStmts_project_body_closure_noMapping_ofIR_globalDefaults
    (nativeRuntimeDispatcherFuel irContract) spec selectors irContract tx
    initialState observableSlots dispatcher nativeYul hCompile hSupported
    hNoMapping hStaticParams hSafeBodies hLowerDispatcher hChainId
    hBlobBaseFee hNoHeader hProject hMatch
...
private theorem layers2_3_ir_matches_native_evmYulLean_of_generated_dispatcherStmts_positive_body_closure_mapping_reserved_ofIR_environment_canonicalFuel
    ...
    nativeResultsMatchOn observableSlots (interpretIR irContract tx initialState)
      (Compiler.Proofs.YulGeneration.Backends.Native.interpretIRRuntimeNative
        (nativeRuntimeFuel irContract) irContract tx initialState observableSlots) :=
  layers2_3_ir_matches_native_evmYulLean_of_generated_dispatcherStmts_positive_body_closure_mapping_reserved_ofIR_environment
    (nativeRuntimeDispatcherFuel irContract) spec selectors irContract tx
    initialState observableSlots dispatcher nextSwitchId hCompile hSupported
    hMapping hStaticParams hSafeBodies hLowerDispatcher hChainId
    hBlobBaseFee hNoHeader hNativeDispatcherExec
...
private theorem layers2_3_ir_matches_native_evmYulLean_of_generated_dispatcherStmts_positive_body_closure_mapping_reserved_ofIR_globalDefaults_canonicalFuel
    ...
    nativeResultsMatchOn observableSlots (interpretIR irContract tx initialState)
      (Compiler.Proofs.YulGeneration.Backends.Native.interpretIRRuntimeNative
        (nativeRuntimeFuel irContract) irContract tx initialState observableSlots) :=
  layers2_3_ir_matches_native_evmYulLean_of_generated_dispatcherStmts_positive_body_closure_mapping_reserved_ofIR_globalDefaults
    (nativeRuntimeDispatcherFuel irContract) spec selectors irContract tx
    initialState observableSlots dispatcher nextSwitchId hCompile hSupported
    hMapping hStaticParams hSafeBodies hLowerDispatcher hChainId
    hBlobBaseFee hNoHeader hNativeDispatcherExec
...
private theorem layers2_3_ir_matches_native_evmYulLean_of_generated_dispatcherStmts_project_body_closure_mapping_reserved_ofIR_globalDefaults_canonicalFuel
    ...
    nativeResultsMatchOn observableSlots (interpretIR irContract tx initialState)
      (Compiler.Proofs.YulGeneration.Backends.Native.interpretIRRuntimeNative
        (nativeRuntimeFuel irContract) irContract tx initialState observableSlots) :=
  layers2_3_ir_matches_native_evmYulLean_of_generated_dispatcherStmts_project_body_closure_mapping_reserved_ofIR_globalDefaults
    (nativeRuntimeDispatcherFuel irContract) spec selectors irContract tx
    initialState observableSlots dispatcher nextSwitchId nativeYul hCompile
    hSupported hMapping hStaticParams hSafeBodies hLowerDispatcher hChainId
    hBlobBaseFee hNoHeader hProject hMatch
...
private theorem layers2_3_ir_matches_native_evmYulLean_of_generated_lowered_runtime_dispatcherStmts_project_body_closure_noMapping
  ...
  layer3_contract_preserves_semantics_native_of_compiled_generated_lowered_runtime_dispatcherStmts_project_body_closure_noMapping
    fuel' spec selectors irContract tx initialState observableSlots nativeContract
    nativeYul hCompile hSupported hNoMapping hStaticParams hSafeBodies hLower
    hEnv hProject hMatch
...
private theorem layers2_3_ir_matches_native_evmYulLean_of_generated_lowered_runtime_dispatcherStmts_project_body_closure_mapping_reserved
  ...
  layer3_contract_preserves_semantics_native_of_compiled_generated_lowered_runtime_dispatcherStmts_project_body_closure_mapping_reserved
    fuel' spec selectors irContract tx initialState observableSlots nativeContract
    nativeYul hCompile hSupported hMapping hStaticParams hSafeBodies hLower hEnv
    hProject hMatch
...
private theorem layers2_3_ir_matches_native_evmYulLean_of_generated_lowered_runtime_dispatcherStmts_positive_body_closure_noMapping_ofIR_environment
    ...
    nativeResultsMatchOn observableSlots (interpretIR irContract tx initialState)
      (Compiler.Proofs.YulGeneration.Backends.Native.interpretIRRuntimeNative
        (Nat.succ fuel') irContract tx initialState observableSlots) :=
  layers2_3_ir_matches_native_evmYulLean_of_generated_lowered_runtime_dispatcherStmts_positive_body_closure_noMapping
    fuel' spec selectors irContract tx initialState observableSlots nativeContract
    hCompile hSupported hNoMapping hStaticParams hSafeBodies hLower
    (Compiler.Proofs.YulGeneration.Backends.Native.validateNativeRuntimeEnvironment_ofIR_representableEnvironment
      (Compiler.emitYul irContract).runtimeCode tx hChainId hBlobBaseFee
      hNoHeader)
    hNativeDispatcherExec
...
private theorem layers2_3_ir_matches_native_evmYulLean_of_generated_lowered_runtime_dispatcherStmts_project_body_closure_noMapping_ofIR_environment
    ...
    nativeResultsMatchOn observableSlots (interpretIR irContract tx initialState)
      (Compiler.Proofs.YulGeneration.Backends.Native.interpretIRRuntimeNative
        (Nat.succ fuel') irContract tx initialState observableSlots) :=
  layers2_3_ir_matches_native_evmYulLean_of_generated_lowered_runtime_dispatcherStmts_project_body_closure_noMapping
    fuel' spec selectors irContract tx initialState observableSlots nativeContract
    nativeYul hCompile hSupported hNoMapping hStaticParams hSafeBodies hLower
    (Compiler.Proofs.YulGeneration.Backends.Native.validateNativeRuntimeEnvironment_ofIR_representableEnvironment
      (Compiler.emitYul irContract).runtimeCode tx hChainId hBlobBaseFee
...
private theorem layers2_3_ir_matches_native_evmYulLean_of_generated_lowered_runtime_dispatcherStmts_positive_body_closure_mapping_reserved_ofIR_environment
    ...
    nativeResultsMatchOn observableSlots (interpretIR irContract tx initialState)
      (Compiler.Proofs.YulGeneration.Backends.Native.interpretIRRuntimeNative
        (Nat.succ fuel') irContract tx initialState observableSlots) :=
  layers2_3_ir_matches_native_evmYulLean_of_generated_lowered_runtime_dispatcherStmts_positive_body_closure_mapping_reserved
    fuel' spec selectors irContract tx initialState observableSlots nativeContract
    hCompile hSupported hMapping hStaticParams hSafeBodies hLower
    (Compiler.Proofs.YulGeneration.Backends.Native.validateNativeRuntimeEnvironment_ofIR_representableEnvironment
      (Compiler.emitYul irContract).runtimeCode tx hChainId hBlobBaseFee
      hNoHeader)
    hNativeDispatcherExec
...
private theorem layers2_3_ir_matches_native_evmYulLean_of_generated_lowered_runtime_dispatcherStmts_project_body_closure_mapping_reserved_ofIR_environment
    ...
    (hLower : Compiler.Proofs.YulGeneration.Backends.lowerRuntimeContractNative
      (Compiler.emitYul irContract).runtimeCode = .ok nativeContract)
    (hChainId : Compiler.Proofs.YulGeneration.Backends.Native.nativeChainIdRepresentable
      tx.chainId = true)
    (hBlobBaseFee : Compiler.Proofs.YulGeneration.Backends.Native.nativeBlobBaseFeeRepresentable
      tx.blobBaseFee = true)
    (hNoHeader :
      Compiler.Proofs.YulGeneration.Backends.Native.nativeRuntimePathUsesUnsupportedHeaderBuiltin
        (Compiler.emitYul irContract).runtimeCode (YulTransaction.ofIR tx) =
    ...
    nativeResultsMatchOn observableSlots (interpretIR irContract tx initialState)
      (Compiler.Proofs.YulGeneration.Backends.Native.interpretIRRuntimeNative
        (Nat.succ fuel') irContract tx initialState observableSlots) :=
  layers2_3_ir_matches_native_evmYulLean_of_generated_lowered_runtime_dispatcherStmts_project_body_closure_mapping_reserved
    fuel' spec selectors irContract tx initialState observableSlots nativeContract
    nativeYul hCompile hSupported hMapping hStaticParams hSafeBodies hLower
    (Compiler.Proofs.YulGeneration.Backends.Native.validateNativeRuntimeEnvironment_ofIR_representableEnvironment
      (Compiler.emitYul irContract).runtimeCode tx hChainId hBlobBaseFee
      hNoHeader)
    hProject hMatch
...
private theorem layers2_3_ir_matches_native_evmYulLean_of_generated_lowered_runtime_dispatcherStmts_positive_body_closure_noMapping_ofIR_environment_canonicalFuel
    ...
    nativeResultsMatchOn observableSlots (interpretIR irContract tx initialState)
      (Compiler.Proofs.YulGeneration.Backends.Native.interpretIRRuntimeNative
        (nativeRuntimeFuel irContract) irContract tx initialState observableSlots) :=
  layers2_3_ir_matches_native_evmYulLean_of_generated_lowered_runtime_dispatcherStmts_positive_body_closure_noMapping_ofIR_environment
    (nativeRuntimeDispatcherFuel irContract) spec selectors irContract tx
    initialState observableSlots nativeContract hCompile hSupported hNoMapping
    hStaticParams hSafeBodies hLower hChainId hBlobBaseFee hNoHeader
    hNativeDispatcherExec
...
private theorem layers2_3_ir_matches_native_evmYulLean_of_generated_lowered_runtime_dispatcherStmts_positive_body_closure_noMapping_ofIR_globalDefaults_canonicalFuel
    ...
    nativeResultsMatchOn observableSlots (interpretIR irContract tx initialState)
      (Compiler.Proofs.YulGeneration.Backends.Native.interpretIRRuntimeNative
        (nativeRuntimeFuel irContract) irContract tx initialState observableSlots) :=
  layers2_3_ir_matches_native_evmYulLean_of_generated_lowered_runtime_dispatcherStmts_positive_body_closure_noMapping_ofIR_globalDefaults
    (nativeRuntimeDispatcherFuel irContract) spec selectors irContract tx
    initialState observableSlots nativeContract hCompile hSupported hNoMapping
    hStaticParams hSafeBodies hLower hChainId hBlobBaseFee hNoHeader
    hNativeDispatcherExec
...
private theorem layers2_3_ir_matches_native_evmYulLean_of_generated_lowered_runtime_dispatcherStmts_project_body_closure_noMapping_ofIR_environment_canonicalFuel
    ...
    nativeResultsMatchOn observableSlots (interpretIR irContract tx initialState)
      (Compiler.Proofs.YulGeneration.Backends.Native.interpretIRRuntimeNative
        (nativeRuntimeFuel irContract) irContract tx initialState observableSlots) :=
  layers2_3_ir_matches_native_evmYulLean_of_generated_lowered_runtime_dispatcherStmts_project_body_closure_noMapping_ofIR_environment
    (nativeRuntimeDispatcherFuel irContract) spec selectors irContract tx
    initialState observableSlots nativeContract nativeYul hCompile hSupported
    hNoMapping hStaticParams hSafeBodies hLower hChainId hBlobBaseFee hNoHeader
    hProject hMatch
...
private theorem layers2_3_ir_matches_native_evmYulLean_of_generated_lowered_runtime_dispatcherStmts_project_body_closure_noMapping_ofIR_globalDefaults_canonicalFuel
    ...
    nativeResultsMatchOn observableSlots (interpretIR irContract tx initialState)
      (Compiler.Proofs.YulGeneration.Backends.Native.interpretIRRuntimeNative
        (nativeRuntimeFuel irContract) irContract tx initialState observableSlots) :=
  layers2_3_ir_matches_native_evmYulLean_of_generated_lowered_runtime_dispatcherStmts_project_body_closure_noMapping_ofIR_globalDefaults
    (nativeRuntimeDispatcherFuel irContract) spec selectors irContract tx
    initialState observableSlots nativeContract nativeYul hCompile hSupported
    hNoMapping hStaticParams hSafeBodies hLower hChainId hBlobBaseFee hNoHeader
    hProject hMatch
...
private theorem layers2_3_ir_matches_native_evmYulLean_of_generated_lowered_runtime_dispatcherStmts_positive_body_closure_mapping_reserved_ofIR_environment_canonicalFuel
    ...
    nativeResultsMatchOn observableSlots (interpretIR irContract tx initialState)
      (Compiler.Proofs.YulGeneration.Backends.Native.interpretIRRuntimeNative
        (nativeRuntimeFuel irContract) irContract tx initialState observableSlots) :=
  layers2_3_ir_matches_native_evmYulLean_of_generated_lowered_runtime_dispatcherStmts_positive_body_closure_mapping_reserved_ofIR_environment
    (nativeRuntimeDispatcherFuel irContract) spec selectors irContract tx
    initialState observableSlots nativeContract hCompile hSupported hMapping
    hStaticParams hSafeBodies hLower hChainId hBlobBaseFee hNoHeader
    hNativeDispatcherExec
...
private theorem layers2_3_ir_matches_native_evmYulLean_of_generated_lowered_runtime_dispatcherStmts_positive_body_closure_mapping_reserved_ofIR_globalDefaults_canonicalFuel
    ...
    nativeResultsMatchOn observableSlots (interpretIR irContract tx initialState)
      (Compiler.Proofs.YulGeneration.Backends.Native.interpretIRRuntimeNative
        (nativeRuntimeFuel irContract) irContract tx initialState observableSlots) :=
  layers2_3_ir_matches_native_evmYulLean_of_generated_lowered_runtime_dispatcherStmts_positive_body_closure_mapping_reserved_ofIR_globalDefaults
    (nativeRuntimeDispatcherFuel irContract) spec selectors irContract tx
    initialState observableSlots nativeContract hCompile hSupported hMapping
    hStaticParams hSafeBodies hLower hChainId hBlobBaseFee hNoHeader
    hNativeDispatcherExec
...
private theorem layers2_3_ir_matches_native_evmYulLean_of_generated_lowered_runtime_dispatcherStmts_project_body_closure_mapping_reserved_ofIR_environment_canonicalFuel
    ...
    nativeResultsMatchOn observableSlots (interpretIR irContract tx initialState)
      (Compiler.Proofs.YulGeneration.Backends.Native.interpretIRRuntimeNative
        (nativeRuntimeFuel irContract) irContract tx initialState observableSlots) :=
  layers2_3_ir_matches_native_evmYulLean_of_generated_lowered_runtime_dispatcherStmts_project_body_closure_mapping_reserved_ofIR_environment
    (nativeRuntimeDispatcherFuel irContract) spec selectors irContract tx
    initialState observableSlots nativeContract nativeYul hCompile hSupported
    hMapping hStaticParams hSafeBodies hLower hChainId hBlobBaseFee hNoHeader
    hProject hMatch
...
private theorem layers2_3_ir_matches_native_evmYulLean_of_generated_lowered_runtime_dispatcherStmts_project_body_closure_mapping_reserved_ofIR_globalDefaults_canonicalFuel
    ...
    nativeResultsMatchOn observableSlots (interpretIR irContract tx initialState)
      (Compiler.Proofs.YulGeneration.Backends.Native.interpretIRRuntimeNative
        (nativeRuntimeFuel irContract) irContract tx initialState observableSlots) :=
  layers2_3_ir_matches_native_evmYulLean_of_generated_lowered_runtime_dispatcherStmts_project_body_closure_mapping_reserved_ofIR_globalDefaults
    (nativeRuntimeDispatcherFuel irContract) spec selectors irContract tx
    initialState observableSlots nativeContract nativeYul hCompile hSupported
    hMapping hStaticParams hSafeBodies hLower hChainId hBlobBaseFee hNoHeader
    hProject hMatch
...
/-- The concrete SimpleStorage native witness is the helper-free native
contract produced by any successful lowering of the generated dispatcher. -/
private theorem simpleStorageNativeContract_eq_nativeContractOfDispatcher_of_lowerStmtsNative
    {dispatcher : List EvmYul.Yul.Ast.Stmt}
    (hLowerDispatcher : Compiler.Proofs.YulGeneration.Backends.lowerStmtsNative
      [Compiler.CodegenCommon.buildSwitch simpleStorageIRContract.functions none none] =
        .ok dispatcher) :
    Compiler.SimpleStorageNativeWitness.nativeContract =
      nativeContractOfDispatcher dispatcher := by
  rcases
    Compiler.Proofs.YulGeneration.Backends.Native.lowerRuntimeContractNative_emitYul_noMapping_ok_dispatcher
      simpleStorageIRContract Compiler.SimpleStorageNativeWitness.nativeContract
      rfl rfl rfl rfl
      Compiler.SimpleStorageNativeWitness.lowerRuntimeContractNative_eq with
    ⟨dispatcher', hLowerDispatcher', hNative⟩
  rw [hLowerDispatcher] at hLowerDispatcher'
  simp only [Except.ok.injEq] at hLowerDispatcher'
  subst dispatcher'
  exact hNative
...
private theorem simpleStorage_endToEnd_native_evmYulLean_of_lowered_runtime_dispatcherStmts_match
    ...
    nativeResultsMatchOn observableSlots
      ...
      (Compiler.Proofs.YulGeneration.Backends.Native.interpretIRRuntimeNative
        (sizeOf (Compiler.emitYul simpleStorageIRContract).runtimeCode + 1)
        simpleStorageIRContract tx initialState observableSlots) := by
  apply simpleStorage_endToEnd_native_evmYulLean_of_positive_dispatcherExec_match
  ...
  rcases
    Compiler.Proofs.YulGeneration.Backends.Native.lowerRuntimeContractNative_emitYul_noMapping_ok_dispatcher
      ...
      Compiler.SimpleStorageNativeWitness.lowerRuntimeContractNative_eq with
    ⟨dispatcher, hLowerDispatcher, hNative⟩
  have hMatch := hNativeDispatcherExec dispatcher hLowerDispatcher
  simpa [hNative] using hMatch
...
theorem simpleStorage_endToEnd_native_evmYulLean
  ...
  simpleStorage_endToEnd_native_evmYulLean_of_lowered_runtime_dispatcherStmts_match
    ...
    (by
      ...
      have hNativeEq :=
        simpleStorageNativeContract_eq_nativeContractOfDispatcher_of_lowerStmtsNative
          hLowerDispatcher
