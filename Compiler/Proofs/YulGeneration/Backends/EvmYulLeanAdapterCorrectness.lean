/-
  ...
  2. **For init-hoisting**: EVMYulLean's `For` has no init block; the adapter
     ...
     because `legacyExecYulFuel` executes the init block once before entering the
...
identically in `legacyExecYulFuel`: they evaluate `value` and call `state.setVar`.
...
The key insight is that `legacyExecYulFuel` handles `for_` by:
