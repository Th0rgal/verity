...
private def isAccrueInterestTimestampWriteBlock
...
/-- Smoke test: checked arithmetic rewrite canonicalizes total supply share addition in accrue-interest flow. -/
example :
    let input : YulObject :=
      { name := "Main"
        deployCode := []
...
example :
    let input : YulObject :=
      { name := "Main"
        ...
        runtimeCode :=
          [ .funcDef "fun_accrueInterest" [] []
              [ .let_ "newTotalSupplyShares_9" (.call "add" [.ident "totalSupplyShares_2", .ident "feeShares_1"]) ]
          , .block [ .expr (.call "fun_accrueInterest" []) ]
          ] }
    let report := runPatchPassWithObjects
      { enabled := true
...
example :
    let input : YulObject :=
      { name := "Main"
        ...
        runtimeCode :=
          [ .funcDef "fun_accrueInterest" [] []
              [ .let_ "newTotalSupplyShares" (.call "add" [.ident "totalSupplySharesAlt", .ident "feeShares"]) ]
          , .block [ .expr (.call "fun_accrueInterest" []) ]
          ] }
    let report := runPatchPassWithObjects
      { enabled := true
...
example :
      ...
      { name := "Main"
        ...
        runtimeCode :=
          [ .funcDef "fun_accrueInterest" [] []
              [ .expr
                  (.call "sstore"
                    [ .ident "__compat_mapping_slot_17"
                    , .call "or"
                        [ .ident "__compat_slot_cleared"
                        , .call "shl" [.lit 128, .ident "__compat_packed"]
                        ]
                    ])
              ]
          , .block [ .expr (.call "fun_accrueInterest" []) ]
    ...
    let called := callNamesInStmts report.patched.runtimeCode
    report.manifest.map (fun m => m.patchName) = ["solc-compat-rewrite-accrue-interest-checked-arithmetic"] &&
      hasTopLevelFunctionNamed report.patched.runtimeCode "update_storage_value_offsett_uint128_to_uint128" &&
      called.any (fun name => name = "update_storage_value_offsett_uint128_to_uint128") = true := by
  native_decide
      ...
      { name := "Main"
        ...
        runtimeCode :=
          [ .funcDef "fun_accrueInterest" [] []
              ...
              , .if_ (.call "iszero" [.ident "callSuccess"])
                  [ .let_ "__ecwr_rds" (.call "returndatasize" [])
                  , .expr (.call "returndatacopy" [.lit 0, .lit 0, .ident "__ecwr_rds"])
                  , .expr (.call "revert" [.lit 0, .ident "__ecwr_rds"])
                  ]
...
/-- Smoke test: checked arithmetic rewrite normalizes block-local IRM failure forwarding
    (`let __ecwr_success* := call(... __compat_alloc_ptr* ...)` + inline forwarding with
    arbitrary temp/pointer names + `lt(returndatasize(), 32)`) to helper-backed
    `extract_returndata()` forwarding. -/
example :
    let input : YulObject :=
      { name := "Main"
        deployCode := []
        runtimeCode :=
          [ .funcDef "fun_accrueInterest" [] []
              [ .let_ "__compat_alloc_ptr_9" (.ident "ptr")
              , .let_ "__ecwr_success_9"
                  (.call "call"
                    [ .call "gas" []
                    , .ident "irm"
                    , .lit 0
              ...
              , .if_ (.call "iszero" [.ident "__ecwr_success_9"])
                  [ .let_ "__tmp_pos_9" (.call "mload" [.ident "__compat_mem_base"])
                  , .expr (.call "returndatacopy" [.ident "__tmp_pos_9", .lit 0, .call "returndatasize" []])
                  , .expr (.call "revert" [.ident "__tmp_pos_9", .call "returndatasize" []])
                  ]
              , .if_ (.call "lt" [.call "returndatasize" [], .lit 32])
                  [ .expr (.call "revert" [.lit 0, .lit 0]) ]
              ]
...
/-- Smoke test: block-local IRM forwarding normalization stays out-of-scope when
    `returndatacopy`/`revert` do not use the same temp pointer. -/
example :
    let input : YulObject :=
      { name := "Main"
        deployCode := []
        runtimeCode :=
          [ .funcDef "fun_accrueInterest" [] []
              [ .let_ "__compat_alloc_ptr_11" (.ident "ptr")
              , .let_ "__ecwr_success_11"
                  (.call "call"
                    [ .call "gas" []
                    , .ident "irm"
                    , .lit 0
                    , .lit 0
                    , .lit 356
                    , .ident "__compat_alloc_ptr_11"
                    , .lit 32
                    ])
              , .expr (.call "mstore" [.lit 0, .call "mload" [.ident "__compat_alloc_ptr_11"]])
              , .if_ (.call "iszero" [.ident "__ecwr_success_11"])
                  [ .let_ "__tmp_pos_11" (.call "mload" [.lit 64])
                  , .expr (.call "returndatacopy" [.ident "__tmp_pos_11", .lit 0, .call "returndatasize" []])
                  , .expr (.call "revert" [.ident "__tmp_other_11", .call "returndatasize" []])
                  ]
              , .if_ (.call "lt" [.call "returndatasize" [], .lit 32])
                  [ .expr (.call "revert" [.lit 0, .lit 0]) ]
              ]
...
/-- Smoke test: full IRM prelude shape (`mload(64)` + finalize_* + call + inline revert forwarding)
    normalizes to helper-backed `extract_returndata()` forwarding. -/
example :
    let input : YulObject :=
      { name := "Main"
        deployCode := []
        runtimeCode :=
          [ .funcDef "fun_accrueInterest" [] []
              [ .let_ "__compat_alloc_ptr_22" (.call "mload" [.lit 64])
              , .expr (.call "finalize_allocation_27020" [.ident "__compat_alloc_ptr_22"])
              , .expr (.call "finalize_allocation_27033" [.ident "__compat_alloc_ptr_22"])
              , .expr (.call "finalize_allocation" [.ident "__compat_alloc_ptr_22", .lit 32])
              , .let_ "__ecwr_success_22"
                  (.call "call"
                    [ .call "gas" []
                    , .ident "irm"
                    , .lit 0
...
/-- Smoke test: full IRM prelude normalization stays out-of-scope when finalize/call alloc pointers differ. -/
example :
    let input : YulObject :=
      { name := "Main"
        deployCode := []
        runtimeCode :=
          [ .funcDef "fun_accrueInterest" [] []
              ...
              , .expr (.call "mstore" [.lit 0, .call "mload" [.ident "__compat_alloc_ptr_mismatch"]])
              , .if_ (.call "iszero" [.ident "__ecwr_success_22"])
                  [ .let_ "pos" (.call "mload" [.lit 64])
                  , .expr (.call "returndatacopy" [.ident "pos", .lit 0, .call "returndatasize" []])
                  , .expr (.call "revert" [.ident "pos", .call "returndatasize" []])
...
/-- Smoke test: block-local IRM forwarding normalization stays out-of-scope when
    alloc-pointer base name is not `__compat_alloc_ptr*`. -/
example :
    let input : YulObject :=
      { name := "Main"
        deployCode := []
        runtimeCode :=
          [ .funcDef "fun_accrueInterest" [] []
              [ .let_ "__other_alloc_ptr" (.ident "ptr")
              , .let_ "__ecwr_success_9"
                  (.call "call"
                    [ .call "gas" []
                    , .ident "irm"
                    ...
                    , .lit 0
                    , .lit 356
                    , .ident "__other_alloc_ptr"
                    , .lit 32
                    ])
              , .expr (.call "mstore" [.lit 0, .call "mload" [.ident "__other_alloc_ptr"]])
              , .if_ (.call "iszero" [.ident "__ecwr_success_9"])
                  [ .let_ "pos" (.call "mload" [.lit 64])
                  , .expr (.call "returndatacopy" [.ident "pos", .lit 0, .call "returndatasize" []])
                  , .expr (.call "revert" [.ident "pos", .call "returndatasize" []])
...
/-- Smoke test: helper insertion remains referenced+absent only for `extract_returndata`. -/
example :
    let input : YulObject :=
      { name := "Main"
        deployCode := []
    ...
    let topNames := topLevelFunctionNames report.patched.runtimeCode
    report.manifest.map (fun m => m.patchName) = ["solc-compat-rewrite-accrue-interest-checked-arithmetic"] &&
      (topNames.filter (fun name => name = "extract_returndata")).length = 1 := by
  native_decide

/-- Smoke test: checked arithmetic rewrite does not duplicate pre-existing `extract_returndata` helper. -/
example :
    let input : YulObject :=
      { name := "Main"
        deployCode := []
        runtimeCode :=
          [ extractReturndataHelperStmt
          , .funcDef "fun_accrueInterest" [] []
              [ .if_ (.call "iszero" [.ident "__ecwr_success"])
                  [ .let_ "__ecwr_rds" (.call "returndatasize" [])
                  , .expr (.call "returndatacopy" [.lit 0, .lit 0, .ident "__ecwr_rds"])
...
/-- Smoke test: six-arg `fun_accrueInterest` timestamp write block is moved after the IRM guard
    before market-params lowering. -/
example :
    let input : YulObject :=
      { name := "Main"
        deployCode := []
        runtimeCode :=
          [ .funcDef "fun_accrueInterest"
              ...
              , .if_ (.call "iszero" [.call "eq" [.ident "irm_4", .lit 0]])
                  [ .comment "irm branch" ]
              ]
          , .block [ .expr (.call "fun_accrueInterest" [.lit 1, .lit 2, .lit 3, .lit 4, .lit 5, .lit 6]) ]
          ] }
...
/-- Smoke test: timestamp write reordering stays out-of-scope when compat-id aliases diverge
    inside the timestamp write block. -/
example :
    let input : YulObject :=
      { name := "Main"
        deployCode := []
        runtimeCode :=
          [ .funcDef "fun_accrueInterest"
              ...
              , .block
                  ...
                  , .expr (.call "mstore" [.lit 512, .ident "other_id"])
                  , .expr (.call "mstore" [.lit 544, .lit 3])
                  , .let_ "__compat_mapping_slot_1" (.call "keccak256" [.lit 512, .lit 64])
                  , .let_ "__compat_slot_word"
                      (.call "sload"
...
/-- Smoke test: timestamp write reordering is wrapper-safe for a single top-level runtime block. -/
example :
    let input : YulObject :=
      { name := "Main"
        deployCode := []
        runtimeCode :=
          [ .block
              [ .funcDef "fun_accrueInterest"
                  ...
                  , .if_ (.call "iszero" [.call "eq" [.ident "irm", .lit 0]])
                      [ .comment "irm branch" ]
                  ]
              ]
          ] }
...
/-- Smoke test: timestamp write reordering does not force `extract_returndata` helper insertion
