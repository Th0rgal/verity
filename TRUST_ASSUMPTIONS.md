...
- **Status**: 25 pure builtins bridged to EVMYulLean `UInt256` operations; 18 universal pure bridge theorems are now proven in `Compiler/Proofs/YulGeneration/Backends/EvmYulLeanBridgeLemmas.lean` (15 unsigned arithmetic/bitwise + addmod + mulmod + byte). The remaining pure bridge cases (`exp`, `sdiv`, `smod`, `slt`, `sgt`, `sar`, and `signextend`) are still covered by concrete regression checks. Gas is not modeled.
