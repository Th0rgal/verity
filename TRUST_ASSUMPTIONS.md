...
- **Remaining gaps for whole-program retargeting**: (a) 1 unbridged builtin (`mappingSlot`) requires a Phase 3 keccak-semantic bridge. (b) 2 bridge lemmas use `sorry` (smod, sar) — blocked by complex Int↔UInt256 sign/bit semantics, not privacy. (c) Statement-level structural induction lifting expression equivalence to full Yul program execution is not yet proven. (d) No Layer-3-composed theorem yet connects IR → Yul under `.evmYulLean`.
