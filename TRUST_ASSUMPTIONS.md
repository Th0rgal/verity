...
- **Remaining gaps for whole-program retargeting**: (a) 2 bridge lemmas use `sorry` (smod, sar) — blocked by complex Int↔UInt256 sign/bit semantics, not privacy. (b) No proof yet shows compiler-produced IR function/entrypoint bodies satisfy `BridgedStmt`. (c) No Layer-3-composed theorem yet connects IR → Yul under `.evmYulLean`.
