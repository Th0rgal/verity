...
- **Remaining gaps for whole-program retargeting**: (a) 2 bridge lemmas use `sorry` (smod, sar) — blocked by complex Int↔UInt256 sign/bit semantics, not privacy. (b) Structured-control-flow induction for recursive block bodies and `.for_` is not yet proven. (c) No Layer-3-composed theorem yet connects IR → Yul under `.evmYulLean`.
