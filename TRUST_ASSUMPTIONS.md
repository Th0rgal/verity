...
- **Status**: 25 pure builtins bridged to EVMYulLean `UInt256` operations; 20 universal pure bridge theorems are now proven in `Compiler/Proofs/YulGeneration/Backends/EvmYulLeanBridgeLemmas.lean` (15 unsigned arithmetic/bitwise + addmod + mulmod + byte + slt + sgt). The remaining pure bridge cases (`exp`, `sdiv`, `smod`, `sar`, and `signextend`) are still covered by concrete regression checks. Gas is not modeled.
