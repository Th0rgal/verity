...
- **Status (Phase 4: expression-level retargeting + statement-executor scaffolding)**: 25 universal pure bridge theorems (23 fully proven, 2 with sorry-dependent core equivalences) in `Compiler/Proofs/YulGeneration/Backends/EvmYulLeanBridgeLemmas.lean`. All pure bridge cases are now covered by universal symbolic lemmas. Additionally, 10 context/env/storage builtin bridge theorems and 1 fallthrough (none) lemma bring the total to 35 of 36 builtins bridged. The retargeting module `EvmYulLeanRetarget.lean` proves the pointwise theorem `backends_agree_on_bridged_builtins` and the expression-level theorem `evalYulExpr_evmYulLean_eq_on_bridged` for `BridgedExpr` expressions (dispatch proof is sorry-free). It also defines the backend-parameterized statement executor `execYulFuelWithBackend` and proves `execYulFuelWithBackend_verity_eq`, so the new executor is known to recover existing Verity semantics at `.verity`; it does not yet prove `.verity = .evmYulLean` statement-level equivalence. The 2 sorry-dependent core equivalences are smod/sar. Gas is not modeled.
- **Trust boundary (expression-level)**: For any `BridgedExpr`, the trust assumption moves from "Verity's custom builtin implementations are correct" to "EVMYulLean's execution model matches the EVM" (backed by upstream Ethereum conformance tests).
...
- **Remaining gaps for whole-program retargeting**: (a) 1 unbridged builtin (`mappingSlot`) requires a Phase 3 keccak-semantic bridge. (b) 2 bridge lemmas use `sorry` (smod, sar) — blocked by complex Int↔UInt256 sign/bit semantics, not privacy. (c) Statement-level structural induction over a bridged-statement predicate and `execYulFuelWithBackend` is not yet proven. (d) No Layer-3-composed theorem yet connects IR → Yul under `.evmYulLean`.
