...
**Details**:
- **Lean proof model**: Mapping slots are routed through `Compiler/Proofs/MappingSlot.lean`, whose current backend delegates to the tagged model in `Compiler/Proofs/MappingEncoding.lean` (`mappingTag = 2^256`). A mapping access is encoded as `mappingTag + normalize(baseSlot) * 2^256 + (key % 2^256)`, so mapping slots are disjoint from direct storage slots in the proof model. Runtime routing of `sload`/`sstore` is also centralized there via `abstractLoadStorageOrMapping` and `abstractStoreStorageOrMapping`.
