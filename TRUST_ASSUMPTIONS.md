...
- **Remaining gaps for whole-program retargeting**: (a) 2 bridge lemmas use `sorry` (smod, sar) — blocked by complex Int↔UInt256 sign/bit semantics, not privacy. (b) Body closure is only proved for scalar calldata parameter prologues plus the static scalar leaf-load helper; full compiler-produced IR function/entrypoint bodies still need `BridgedStmt` closure. (c) No Layer-3-composed theorem yet connects IR → Yul under `.evmYulLean`.
