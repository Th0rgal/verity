...
- **Status**: 25 pure builtins bridged to EVMYulLean `UInt256` operations; 25 universal pure bridge theorems (22 fully proven, 3 with sorry-dependent core equivalences) in `Compiler/Proofs/YulGeneration/Backends/EvmYulLeanBridgeLemmas.lean` (15 unsigned arithmetic/bitwise + addmod + mulmod + byte + slt + sgt + sdiv + smod + exp + sar + signextend). All pure bridge cases are now covered by universal symbolic lemmas. Gas is not modeled.
