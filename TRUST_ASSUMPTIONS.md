...
- **Remaining gaps**: 2 unbridged builtins (`sload`, `mappingSlot`) require Phase 3 state bridge. 4 bridge lemmas use `sorry` (sdiv, smod, sar, signextend) pending upstream changes to private definitions. The `backends_agree_on_bridged_builtins` dispatch theorem is now sorry-free (proven by composing 34 per-builtin bridge helpers).
