...
theorem increment_meets_spec (s : ContractState) :
  ...
  · intro slot h_neq
    have h_inc :
        ((increment).run s).snd.storage slot =
          (if slot = 0 then EVM.Uint256.add (s.storage 0) 1 else s.storage slot) := by
      simp [increment, count, getStorage, setStorage, Contract.run, DumbContracts.bind, Bind.bind,
        ContractResult.snd]
    simpa [h_neq] using h_inc
...
theorem decrement_meets_spec (s : ContractState) :
  ...
  · intro slot h_neq
    have h_dec :
        ((decrement).run s).snd.storage slot =
          (if slot = 0 then EVM.Uint256.sub (s.storage 0) 1 else s.storage slot) := by
      simp [decrement, count, getStorage, setStorage, Contract.run, DumbContracts.bind, Bind.bind,
        ContractResult.snd]
    simpa [h_neq] using h_dec
