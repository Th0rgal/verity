...
theorem increment_decrement_meets_cancel (s : ContractState) :
  let s' := ((increment).run s).snd
  let s'' := ((decrement).run s').snd
  Contracts.Counter.Spec.increment_decrement_cancel s s'' := by
  simp [Contracts.Counter.Spec.increment_decrement_cancel]
  exact Contracts.Counter.Proofs.increment_decrement_cancel s
