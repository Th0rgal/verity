...
namespace Contracts.Reentrancy.Example
...
@[simp] theorem modulus_sub_max :
  ...
  have h : DumbContracts.Core.Uint256.modulus = DumbContracts.EVM.MAX_UINT256 + 1 := by
    symm
...
theorem vulnerable_attack_exists :
  ∃ (s : ContractState),
    s.storageMap balances.slot "attacker" = (DumbContracts.EVM.MAX_UINT256 : Uint256) ∧
    s.storage totalSupply.slot = (DumbContracts.EVM.MAX_UINT256 : Uint256) ∧
  ...
  let s : ContractState :=
    { storage := fun slot => if slot == totalSupply.slot then (DumbContracts.EVM.MAX_UINT256 : Uint256) else 0
    , storageAddr := fun _ => ""
  ...
  ·
    have h_cond : decide ((DumbContracts.EVM.MAX_UINT256 : Uint256) ≥ (DumbContracts.EVM.MAX_UINT256 : Uint256)) = true := by
      simp
...
end Contracts.Reentrancy.Example
