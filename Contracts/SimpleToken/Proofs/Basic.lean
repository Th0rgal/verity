...
open DumbContracts
open Contracts.SimpleToken (constructor mint transfer balanceOf getTotalSupply getOwner isOwner)
open Contracts.SimpleToken.Spec hiding owner balances totalSupply
...
theorem getMapping_reads_balance (s : ContractState) (addr : Address) :
  ...
  simp [getMapping, Contracts.SimpleToken.balances]
...
theorem getStorage_reads_supply (s : ContractState) :
  ...
  simp [getStorage, Contracts.SimpleToken.totalSupply]
...
theorem getStorageAddr_reads_owner (s : ContractState) :
  ...
  simp [getStorageAddr, Contracts.SimpleToken.owner]
...
theorem constructor_meets_spec (s : ContractState) (initialOwner : Address) :
  ...
  · intro slot h_neq
    have h_addr :
        ((constructor initialOwner).run s).snd.storageAddr slot =
          (if slot = 0 then initialOwner else s.storageAddr slot) := by
      simp [constructor, Contracts.SimpleToken.owner, Contracts.SimpleToken.totalSupply,
        setStorageAddr, setStorage, Contract.run, DumbContracts.bind, Bind.bind, ContractResult.snd]
    simpa [h_neq] using h_addr
  constructor
  · intro slot h_neq
    have h_storage :
        ((constructor initialOwner).run s).snd.storage slot =
          (if slot = 2 then 0 else s.storage slot) := by
      simp [constructor, Contracts.SimpleToken.owner, Contracts.SimpleToken.totalSupply,
        setStorageAddr, setStorage, Contract.run, DumbContracts.bind, Bind.bind, ContractResult.snd]
    simpa [h_neq] using h_storage
  constructor
  · rfl
  constructor
  · rfl
...
theorem isOwner_true_when_owner (s : ContractState) (h : s.sender = s.storageAddr 0) :
  ...
  simp [isOwner, msgSender, getStorageAddr, Contracts.SimpleToken.owner, Contract.run,
    DumbContracts.bind, DumbContracts.pure, Bind.bind, Pure.pure, ContractResult.fst, h, beq_iff_eq]

-- Shared unfolding definitions for mint and transfer proofs
private abbrev unfold_defs := [``mint, ``transfer,
  ``Contracts.SimpleToken.onlyOwner, ``isOwner,
  ``Contracts.SimpleToken.owner, ``Contracts.SimpleToken.balances, ``Contracts.SimpleToken.totalSupply,
...
private theorem transfer_unfold (s : ContractState) (to : Address) (amount : Uint256)
  ...
  (transfer to amount).run s = ContractResult.success ()
    { storage := s.storage,
      ...
      msgValue := s.msgValue,
      blockTimestamp := s.blockTimestamp } := by
  simp only [transfer, Contracts.SimpleToken.balances,
...
theorem balanceOf_meets_spec (s : ContractState) (addr : Address) :
  ...
  simp [balanceOf, balanceOf_spec, getMapping, Contracts.SimpleToken.balances]
...
theorem getTotalSupply_meets_spec (s : ContractState) :
  ...
  simp [getTotalSupply, getTotalSupply_spec, getStorage, Contracts.SimpleToken.totalSupply]
...
theorem getOwner_meets_spec (s : ContractState) :
  ...
  simp [getOwner, getOwner_spec, getStorageAddr, Contracts.SimpleToken.owner]
...
theorem constructor_getOwner_correct (s : ContractState) (initialOwner : Address) :
  ...
  simp [constructor, getOwner, setStorageAddr, setStorage, getStorageAddr, Contracts.SimpleToken.owner, Contracts.SimpleToken.totalSupply, Contract.run, DumbContracts.bind, ContractResult.snd, ContractResult.fst]
  rfl
