...
currently has universal equivalence lemmas for 21 of them (`add`, `sub`, `mul`,
`div`, `mod`, `addmod`, `mulmod`, `sdiv`, `lt`, `gt`, `slt`, `sgt`, `eq`, `iszero`, `and`, `or`,
`xor`, `not`, `shl`, `shr`, `byte`), while `exp`, `smod`, `sar`, and
